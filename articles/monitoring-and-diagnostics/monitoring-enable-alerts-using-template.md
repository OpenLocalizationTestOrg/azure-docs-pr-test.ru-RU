...
В этой статье показано, как можно использовать [шаблона Azure Resource Manager](../azure-resource-manager/resource-group-authoring-templates.md) tooconfigure Azure метрики оповещений. Это позволяет tooautomatically установка предупреждений на ресурсы при их создании tooensure правильно отслеживает все ресурсы.

Ниже приведены основные шаги Hello.

1. Создание шаблона в формате JSON, описывающий, как toocreate hello предупреждение.
...
Ниже описано, как toocreate шаблона диспетчера ресурсов для оповещения сама по себе, затем предупреждения во время создания hello другого ресурса.

## <a name="resource-manager-template-for-a-metric-alert"></a>Шаблон Resource Manager для оповещения метрики
toocreate оповещение с помощью шаблона диспетчера ресурсов, создать ресурс типа `Microsoft.Insights/alertRules` и заполните все связанные свойства. Ниже приведен шаблон, который создает правило генерации оповещений.
...
Описание hello схему и свойства для правила оповещения [доступен здесь](https://msdn.microsoft.com/library/azure/dn933805.aspx).

## <a name="resource-manager-template-for-a-resource-with-an-alert"></a>Шаблон Resource Manager для ресурса с оповещением
Оповещение в шаблоне Resource Manager чаще всего используется, чтобы создать оповещение при создании ресурса. Например, может потребоваться tooensure, «ЦП % > 80» настроить правило, каждый раз при развертывании виртуальной машины. toodo это, добавьте правило генерации оповещений hello в качестве ресурса в массиве ресурсов hello шаблона виртуальной Машины и добавить зависимость с помощью hello `dependsOn` идентификатор ресурса виртуальной Машины toohello свойство. Ниже приведен полный пример, в котором ВМ Windows создает и добавляет оповещение, которое уведомляет Администраторы подписки, когда hello использование ЦП выходит за пределы 80%.
...
* [Добавление параметров диагностики](monitoring-enable-diagnostic-logs-using-template.md) tooyour шаблона диспетчера ресурсов
