...
---
# <a name="add-your-custom-domain-name-to-azure-active-directory"></a>Добавление имени личного домена в Azure Active Directory
...
Запустите средство Azure AD Connect, чтобы получить DNS-запись, которая будет использоваться Azure AD при проверке домена. Вы увидите DNS-запись на шаге **Домен Azure AD** в мастере. Узнать, как выглядит этот шаг в мастере, можно [здесь](connect/active-directory-aadconnect-get-started-custom.md#verify-the-azure-ad-domain-selected-for-federation). Если у вас нет средства Azure AD Connect, вы можете [скачать его здесь](http://go.microsoft.com/fwlink/?LinkId=615771).
