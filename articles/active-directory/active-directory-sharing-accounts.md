...
* при создании многопользовательских сред. Вы можете располагать одной локальной учетной записью, которая обладает повышенными привилегиями и может использоваться для установки основных компонентов, администрирования и восстановления (например, локальная учетная запись "глобальный администратор" для Office 365 или учетная запись root в Salesforce).

Обычно такие учетные записи используются совместно путем передачи учетных данных (имени пользователя и пароля) соответствующим лицам либо сохранения их в общем расположении, где к ним могут получить доступ несколько доверенных агентов.
...
Пользователи выполняют вход один раз с учетной записью организации. Это та же учетная запись, которой они регулярно пользуются для доступа к рабочему столу или электронной почте. Они могут видеть и использовать только те приложения, которые им назначены. При использовании общих учетных записей этот список приложений может требовать любого количества общих учетных данных. Конечному пользователю не нужно запоминать или записывать разные учетные данные, которые могут им потребоваться.

Общие учетные записи не только улучшают контроль и повышают удобство работы, они также повышают уровень безопасности. Пользователи, имеющие право на использование учетных данных, не видят сам общий пароль, вместо этого они получают разрешения на использование пароля в рамках оркестрованного потока проверки подлинности. Кроме того, для некоторых приложений с единым входом по паролю можно настроить Azure AD на периодическую смену (обновление) пароля с использованием длинных и сложных паролей, что повышает безопасность учетной записи. Администратор может легко предоставлять или запрещать доступ к приложению, а также определять, кто именно имеет доступ к учетной записи и кто осуществлял доступ к ней в прошлом.

Azure AD поддерживает общие учетные записи для любых пользователей с лицензиями Enterprise Mobility Suite (EMS), "Премиум" или "Базовый" во всех типах приложений с единым входом по паролю. Общие учетные записи можно использовать для любого из тысяч предварительно интегрированных приложений в коллекции приложений, также можно добавить собственное приложение с проверкой подлинности по паролю с помощью [пользовательских приложений единого входа](active-directory-sso-integrate-saas-apps.md).
