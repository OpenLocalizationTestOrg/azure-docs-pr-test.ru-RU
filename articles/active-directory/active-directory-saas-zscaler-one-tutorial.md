...
В этом учебнике вы узнаете, как toointegrate Zscaler One с Azure Active Directory (Azure AD).

Интеграция Zscaler One с Azure AD предоставляет hello следующие преимущества:

- Можно управлять в Azure AD, имеющего доступ tooZscaler один
- Можно включить вашей пользователей tooautomatically get вошедшего tooZscaler одно (Single Sign-On) с помощью своих учетных записей Azure AD
...
tooconfigure интеграция Azure AD с Zscaler One, требуется hello следующих элементов:
...
1. Добавление Zscaler One из галереи hello
2. Настройка и проверка единого входа в Azure AD

## <a name="adding-zscaler-one-from-hello-gallery"></a>Добавление Zscaler One из галереи hello
tooconfigure hello интеграции Zscaler One в Azure AD, вы должны tooadd Zscaler One, из списка tooyour коллекции hello управляемых приложений SaaS.

**tooadd Zscaler One из галереи hello, выполните следующие шаги hello.**
...
4. Введите в поле поиска hello **Zscaler One**.

    ![Создание тестового пользователя Azure AD](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_search.png)

5. В панели результатов hello выберите **Zscaler One**и нажмите кнопку **добавить** кнопку tooadd приложения hello.
...
Для единого входа toowork Azure AD необходима tooknow пользователь аналог какие hello в Zscaler One является tooa в Azure AD. Другими словами связи между пользователя Azure AD и связанных пользователей hello в Zscaler One должен установить toobe.

В Zscaler One, присвойте значение hello hello **имя пользователя** в Azure AD в качестве значения hello hello **Username** tooestablish hello связи.

tooconfigure и теста Azure AD единого входа с Zscaler One, требуются следующие стандартные блоки hello toocomplete:
...
4. **[Создание тестового пользователя Zscaler One](#creating-a-zscaler-one-test-user)**  -toohave аналог Саймон Britta в Zscaler One, представление связанных toohello Azure AD пользователя.
...
В этом разделе включения Azure AD единым входом в портал Azure hello и настройки единого входа в приложение Zscaler One.

**tooconfigure Azure AD единого входа с Zscaler One, выполните следующие шаги hello.**

1. В hello в hello портала Azure **Zscaler One** странице интеграции приложения щелкните **единого входа**.
...
3. На hello **URL-адреса и одного домена Zscaler** выполните следующие шаги hello:

    ![Настройка единого входа](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_url.png)

    В текстовом поле URL-адрес входа hello введите URL-адрес hello, используемых вашей пользователей на toosign tooyour Zscaler One приложения.

    > [!NOTE] 
    > Имеется tooupdate это значение с hello фактический URL-адрес входа. Обратитесь к [группы поддержки Zscaler один клиент](https://www.zscaler.com/company/contact) tooget эти значения.
...
6. На hello **одна конфигурация Zscaler** щелкните **Настройка Zscaler One** tooopen **Настройка входа** окна. Копировать hello **SAML единого входа URL-адрес службы** из hello **краткий справочник.**

    ![Настройка единого входа](./media/active-directory-saas-zscaler-one-tutorial/tutorial_zscalerone_configure.png) 

7. В другом окне браузера войдите в сайт Zscaler One компании tooyour в качестве администратора.
...
Пользователи toolog tooenable Azure AD в tooZscaler один, они должны быть подготовленных tooZscaler один. В случае hello объекта Zscaler One Подготовка выполняется вручную.
...
1. Войдите в tooyour **Zscaler One** клиента.
...
> [!NOTE]
> Можно использовать любые другие Zscaler One пользователя средства создания учетных записей или интерфейсы API, предоставляемые Zscaler One tooprovision учетных записей пользователей Azure AD.

### <a name="assigning-hello-azure-ad-test-user"></a>Назначение hello Azure AD тестового пользователя

В этом разделе включите toouse Britta Simon Azure единого входа путем предоставления доступа tooZscaler один.

![Назначение пользователя][200] 

**tooassign tooZscaler Britta Simon, выполните hello следующие шаги.**

1. В hello портал Azure, откройте представление приложения hello, а затем перейдите toohello представления каталога и перейти слишком**корпоративных приложений** щелкните **все приложения**.

    ![Назначение пользователя][201] 

2. В списке приложений hello выберите **Zscaler One**.
...
При нажатии кнопки hello Zscaler One плитки в панели доступа hello, вы должны получить автоматически вошедшего tooyour Zscaler One приложения.
