...
> [!NOTE]
> Если клиент Azure AD находится в Европе, см. сведения в разделе [Известные проблемы](#known-issues) ниже.
...
-   Выражение, которое сопоставляется с атрибутом AD userPrincipalName, создает имя участника-пользователя из firstName.LastName@contoso.com. Оно также заменяет недопустимые специальные символы.
...
После завершения частей 1–3 можно запустить службу подготовки на портале управления Azure.
...
4. Отдельные события синхронизации, например, какие данные пользователи считывают из Workday, которые впоследствии будут добавлены или обновлены в Active Directory, можно просмотреть на вкладке **Журналы аудита**. **[Подробные инструкции о том, как читать журналы аудита, см. в руководстве об отчетах по подготовке](active-directory-saas-provisioning-reporting.md)**.

5.  В журнале приложений Windows на компьютере агента отображаются все операции, выполняемые с помощью агента.
...
* **Журналы аудита на европейских языках**. На момент выпуска этой предварительной технической версии существует известная проблема с [журналами аудита](active-directory-saas-provisioning-reporting.md), связанная с отсутствием отображения приложений соединителей Workday на [портале Azure](https://portal.azure.com), если клиент Azure AD находится в центре обработки данных в Европе. Решение этой проблемы будет выпущено позже. Следите за выходом обновлений в ближайшем будущем. 
