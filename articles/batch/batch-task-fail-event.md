...
# <a name="task-fail-event"></a>Событие сбоя выполнения задачи
 ...
 Это событие создается, когда задача завершается с ошибкой. Сейчас все ненулевые коды выхода считаются ошибками. Это событие будет создаваться *в дополнение к* задачу выполнения события и может быть toodetect используется, если не удалось выполнить задачу.


 Hello пример hello тела задачи ошибкой события.
