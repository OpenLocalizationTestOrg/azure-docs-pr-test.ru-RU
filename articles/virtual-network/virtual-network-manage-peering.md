...
Узнайте, как создавать, изменять и удалять пиринг в виртуальной сети. Пиринг виртуальных сетей — это механизм подключения между двумя виртуальными сетями, находящимися в одном расположении Azure, через магистральную сеть Azure. После создания пиринга две виртуальные сети по-прежнему управляются как отдельные ресурсы. Ресурсы обеих виртуальных сетей обмениваются данными с такой же задержкой и пропускной способностью, как если бы они были подключены к одной виртуальной сети. Если вы не знакомы с пирингом между виртуальными сетями, мы рекомендуем перед выполнением задач в этом разделе прочесть статью [Пиринг между виртуальными сетями](virtual-network-peering-overview.md) и выполнить инструкции [руководства по созданию пиринга между виртуальными сетями](virtual-network-create-peering.md).
...
>[!NOTE]
>Существует ряд требований, ограничений и рекомендаций для успешного создания пиринга между виртуальными сетями. Прежде чем создавать пиринг, ознакомьтесь со списком [рекомендаций и ограничений](#requirements-and-constraints), а также [требуемых разрешений](#permissions).
>
...
6. <a name="add-peering"></a>В колонке **Add peering** (Добавить пиринг), введите или выберите значения описанных ниже параметров.
    ...
    - **Виртуальная сеть.** Выберите виртуальную сеть, с которой нужно установить пиринг. Можно выбрать виртуальную сеть, созданную с помощью любых моделей развертывания Azure, но она должна быть в том же расположении, что и виртуальная сеть, из которой инициируется пиринг. Чтобы виртуальная сеть отображалась в списке, требуется доступ на чтение. Если виртуальная сеть отображается в списке, но она неактивна, возможно, ее адресное пространство перекрывает адресное пространство исходной виртуальной сети. Если адресные пространства виртуальных сетей перекрываются, пиринг установить не удастся. Если установлен флажок **Идентификатор ресурса**, этот параметр недоступен.
...
- Если пиринговая связь устанавливается между двумя виртуальными сетями, созданными при помощи Resource Manager, ее нужно настроить для каждой из этих виртуальных сетей. При создании пиринга с передачей данных из первой виртуальной сети во вторую его состояние принимает значение *Инициировано*.  При создании пиринга с передачей данных из второй виртуальной сети в первую его состояние принимает значение *Подключено*. При этом для первой виртуальной сети значение состояния также изменяется с *Инициировано* на *Подключено*. Пиринг не будет установлен, пока его состояние для обеих виртуальных сетей не будет иметь значение *Подключено*. 
