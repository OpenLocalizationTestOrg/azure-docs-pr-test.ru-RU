...
| --- | --- | --- |
| SSH-подключение к головному или граничному узлу | `-u 'jdbc:hive2://headnodehost:10001/;transportMode=http'` |
...
1. При запуске Beeline необходимо указать строку подключения к HiveServer2 в кластере HDInsight. Чтобы выполнить команду вне кластера, укажите также имя учетной записи (по умолчанию `admin`) и пароль для входа в кластер. Ниже приведена таблица с форматами строки подключения и необходимыми параметрами.
    ...
    Эта команда запускает клиент Beeline и подключает его к HiveServer2 на головном узле кластера. После выполнения команды отобразится командная строка `jdbc:hive2://headnodehost:10001/>`.
...
Spark предоставляет собственную реализацию HiveServer2, также известную как сервер Thrift Spark. Для разрешения запросов эта служба использует Spark SQL вместо Hive и может обеспечить более высокую производительность в зависимости от запроса.
