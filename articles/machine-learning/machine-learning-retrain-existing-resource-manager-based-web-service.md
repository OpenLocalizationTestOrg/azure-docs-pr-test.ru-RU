...
Чтобы обновить данные службы хранилища Azure, необходимо получить такую информацию, как имя учетной записи хранения, ее ключ и контейнер, с классического портала Azure, а затем изменить соответствующие значения в коде. После запуска эксперимента итоговый рабочий процесс должен быть аналогичен следующему:
...
Найдите объявления *StorageAccountName*, *StorageAccountKey* и *StorageContainerName*, а затем обновите их, используя значения, взятые с классического портала Azure.
...
Ниже приведен пример выходных данных переобучения: ![Выходные данные переобучения][6]
...
Сначала войдите в учетную запись Azure в среде PowerShell, используя командлет [Add-AzureRmAccount](https://msdn.microsoft.com/library/mt619267.aspx).
