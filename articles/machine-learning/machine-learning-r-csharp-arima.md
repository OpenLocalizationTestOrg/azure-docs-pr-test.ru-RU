...
Эта служба принимает аргументы, 4 и вычисляет прогнозы ARIMA hello.
...
Существует несколько способов использования службы hello в автоматическом режиме (пример приложения — [здесь](http://microsoftazuremachinelearning.azurewebsites.net/ArimaForecasting.aspx)).
...
В службах машинного обучения Azure создан пустой эксперимент. Загружен образец входных данных с заранее заданной схемой данных. Схема данных связанного toohello [выполнение скрипта R] [ execute-r-script] модуль, который создает модель прогнозирования hello ARIMA с помощью функции «auto.arima» и «прогноз» из кода R. 
...
Это очень простой пример использования модели прогнозирования ARIMA. Как видно в приведенном выше примере кода hello, перехват ошибок не реализован, и службы hello предполагается, что все переменные hello, непрерывные или положительные значения и hello частоты должно быть целое число больше 1. Длина Hello hello векторов значения даты и следует hello же. переменная приветствия даты должны соблюдаться toohello формат «мм/дд/гггг».

## <a name="faq"></a>Часто задаваемые вопросы
В разделе часто задаваемых вопросов о потреблении hello веб-службы или публикации toomarketplace [здесь](machine-learning-marketplace-faq.md).
