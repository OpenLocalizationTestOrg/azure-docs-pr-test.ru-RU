...
* **&#60;table name>** — имя таблицы, создаваемой в указанной базе данных. Если вы хотите использовать базу данных по умолчанию, то для обращения к таблице можно указать только имя таблицы *&#60;имя_таблицы>* без указания имени базы данных.
...
> [!NOTE]
> Если в таблице TEXTFILE *&#60;database name>.&#60;external textfile table name>* есть секции, команда `SELECT * FROM <database name>.<external textfile table name>`, выполняемая на шаге 3, сохраняет обозначение секции в отдельное поле в возвращаемом наборе данных. Если попытаться вставить эти данные в *&#60;database name>.&#60;ORC table name>*, возникает ошибка, так как в схеме таблицы *&#60;database name>.&#60;ORC table name>* нет отдельного поля для обозначения секции. В этом случае необходимо указать поля, которые требуется вставить в таблицу *&#60;database name>.&#60;ORC table name>*, следующим образом:
>
>
