...
## <a name="download-and-install-azcopy"></a><span data-ttu-id="d76ba-112">Скачивание и установка AzCopy</span><span class="sxs-lookup"><span data-stu-id="d76ba-112">Download and install AzCopy</span></span>
...
### <a name="download-blobs-with-specified-prefix"></a><span data-ttu-id="d76ba-132">Скачивание больших двоичных объектов с указанным префиксом</span><span class="sxs-lookup"><span data-stu-id="d76ba-132">Download blobs with specified prefix</span></span>
...
<span data-ttu-id="d76ba-212">индекс диапазона ключа секции Hello равно 0, если параметр не указан пользователь hello `/PKRS`.</span><span class="sxs-lookup"><span data-stu-id="d76ba-212">hello partition key range index is 0 if hello user does not specify option `/PKRS`.</span></span>
...
<span data-ttu-id="d76ba-222">AzCopy начинается сущностей tooexport параллельных операций, когда пользователь hello указывает параметр `/PKRS`.</span><span class="sxs-lookup"><span data-stu-id="d76ba-222">AzCopy starts concurrent operations tooexport entities when hello user specifies option `/PKRS`.</span></span> <span data-ttu-id="d76ba-223">Каждая операция экспортирует объем данных равный значению объема, указанного в индексе диапазонов ключей секций.</span><span class="sxs-lookup"><span data-stu-id="d76ba-223">Each operation exports one partition key range.</span></span>
...
<span data-ttu-id="d76ba-242">Обратите внимание, что нельзя задать параметр `/PKRS` в случае импорта hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-242">Note that you cannot specify option `/PKRS` in hello import scenario.</span></span> <span data-ttu-id="d76ba-243">В отличие от сценария экспорта hello, в котором необходимо указать параметр `/PKRS` toostart параллельных операций, AzCopy параллельных операций по умолчанию включается при импорте таблицы.</span><span class="sxs-lookup"><span data-stu-id="d76ba-243">Unlike hello export scenario, in which you must specify option `/PKRS` toostart concurrent operations, AzCopy starts concurrent operations by default when you import a table.</span></span> <span data-ttu-id="d76ba-244">количество параллельных операций запуска по умолчанию Hello равно toohello равно числу процессоров core; Тем не менее, можно указать другое количество одновременных с параметром `/NC`.</span><span class="sxs-lookup"><span data-stu-id="d76ba-244">hello default number of concurrent operations started is equal toohello number of core processors; however, you can specify a different number of concurrent with option `/NC`.</span></span> <span data-ttu-id="d76ba-245">Для получения дополнительных сведений введите `AzCopy /?:NC` hello командной строки.</span><span class="sxs-lookup"><span data-stu-id="d76ba-245">For more details, type `AzCopy /?:NC` at hello command line.</span></span>
...
<span data-ttu-id="d76ba-264">AzCopy завершается неудачей, если параметр hello разбиты на две строки, как показано ниже для hello `/sourcekey` параметр:</span><span class="sxs-lookup"><span data-stu-id="d76ba-264">AzCopy fails if you split hello parameter across two lines, as shown here for hello `/sourcekey` parameter:</span></span>
...
<span data-ttu-id="d76ba-282">Если нужно toospecify пользовательское расположение для файла журнала hello:</span><span class="sxs-lookup"><span data-stu-id="d76ba-282">If you want toospecify a custom location for hello journal file:</span></span>
...
### <a name="specify-hello-number-of-concurrent-operations-toostart"></a><span data-ttu-id="d76ba-291">Укажите число hello toostart параллельных операций</span><span class="sxs-lookup"><span data-stu-id="d76ba-291">Specify hello number of concurrent operations toostart</span></span>
...
* <span data-ttu-id="d76ba-303">Подробная справка по командной строке AzCopy: `AzCopy /?`</span><span class="sxs-lookup"><span data-stu-id="d76ba-303">For detailed command-line help for AzCopy: `AzCopy /?`</span></span>
...
<span data-ttu-id="d76ba-332">**Применимо к** большим двоичным объектам, файлам, таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-332">**Applicable to:** Blobs, Files, Tables</span></span>
...
<span data-ttu-id="d76ba-345">Если hello источника ресурсов — это контейнер больших двоичных объектов и подписанный URL-адрес, ни ключ предоставляется, контейнер больших двоичных объектов hello считывается через анонимный доступ.</span><span class="sxs-lookup"><span data-stu-id="d76ba-345">If hello source resource is a blob container, and neither a key nor a SAS is provided, then hello blob container is read via anonymous access.</span></span>
...
<span data-ttu-id="d76ba-377">Задает папку файла журнала для возобновления операции.</span><span class="sxs-lookup"><span data-stu-id="d76ba-377">Specifies a journal file folder for resuming an operation.</span></span>

<span data-ttu-id="d76ba-378">AzCopy всегда поддерживает возобновление, если операция была прервана.</span><span class="sxs-lookup"><span data-stu-id="d76ba-378">AzCopy always supports resuming if an operation has been interrupted.</span></span>

<span data-ttu-id="d76ba-379">Если этот параметр не указан или указан без путь к папке, AzCopy создает файл журнала hello в расположении по умолчанию hello, а именно % LocalAppData%\Microsoft\Azure\AzCopy.</span><span class="sxs-lookup"><span data-stu-id="d76ba-379">If this option is not specified, or it is specified without a folder path, then AzCopy creates hello journal file in hello default location, which is %LocalAppData%\Microsoft\Azure\AzCopy.</span></span>

<span data-ttu-id="d76ba-380">Каждый раз, когда выдачи команды tooAzCopy, он проверяет, существует ли файл журнала в папке по умолчанию hello, или существует ли в папке, указанного с помощью этого параметра.</span><span class="sxs-lookup"><span data-stu-id="d76ba-380">Each time you issue a command tooAzCopy, it checks whether a journal file exists in hello default folder, or whether it exists in a folder that you specified via this option.</span></span> <span data-ttu-id="d76ba-381">Если файл журнала hello не существует в любом месте, AzCopy считает новые операции hello и создает новый файл журнала.</span><span class="sxs-lookup"><span data-stu-id="d76ba-381">If hello journal file does not exist in either place, AzCopy treats hello operation as new and generates a new journal file.</span></span>

<span data-ttu-id="d76ba-382">Если существует файл журнала hello, AzCopy проверяет, соответствует ли hello командной строки, что вы ввод hello командной строки в файле журнала hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-382">If hello journal file does exist, AzCopy checks whether hello command line that you input matches hello command line in hello journal file.</span></span> <span data-ttu-id="d76ba-383">Если две командные строки hello совпадают, AzCopy возобновляет hello незавершенные операции.</span><span class="sxs-lookup"><span data-stu-id="d76ba-383">If hello two command lines match, AzCopy resumes hello incomplete operation.</span></span> <span data-ttu-id="d76ba-384">Если они не совпадают, не запрошенные tooeither перезаписать hello журнала файла toostart новую операцию или toocancel hello текущей операции.</span><span class="sxs-lookup"><span data-stu-id="d76ba-384">If they do not match, you are prompted tooeither overwrite hello journal file toostart a new operation, or toocancel hello current operation.</span></span>

<span data-ttu-id="d76ba-385">файл журнала Hello удаляется после успешного завершения операции hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-385">hello journal file is deleted upon successful completion of hello operation.</span></span>

<span data-ttu-id="d76ba-386">Обратите внимание, что возобновление операции из файла журнала, созданного предыдущей версией AzCopy, не поддерживается.</span><span class="sxs-lookup"><span data-stu-id="d76ba-386">Note that resuming an operation from a journal file created by a previous version of AzCopy is not supported.</span></span>

<span data-ttu-id="d76ba-387">**Применимо к** большим двоичным объектам, файлам, таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-387">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="parameter-file"></a><span data-ttu-id="d76ba-388">/@:"parameter-file"</span><span class="sxs-lookup"><span data-stu-id="d76ba-388">/@:"parameter-file"</span></span>
<span data-ttu-id="d76ba-389">Задает файл, который содержит параметры.</span><span class="sxs-lookup"><span data-stu-id="d76ba-389">Specifies a file that contains parameters.</span></span> <span data-ttu-id="d76ba-390">AzCopy процессы hello параметры в файле hello, как если бы они были указаны в командной строке hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-390">AzCopy processes hello parameters in hello file just as if they had been specified on hello command line.</span></span>

<span data-ttu-id="d76ba-391">В файле ответа можно либо задать несколько параметров в одной строке, либо задать каждый параметр в отдельной строке.</span><span class="sxs-lookup"><span data-stu-id="d76ba-391">In a response file, you can either specify multiple parameters on a single line, or specify each parameter on its own line.</span></span> <span data-ttu-id="d76ba-392">Обратите внимание, что отдельный параметр не может занимать несколько строк.</span><span class="sxs-lookup"><span data-stu-id="d76ba-392">Note that an individual parameter cannot span multiple lines.</span></span>

<span data-ttu-id="d76ba-393">Файлы ответа могут содержать комментарии строки, начинающиеся с символа # hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-393">Response files can include comments lines that begin with hello # symbol.</span></span>

<span data-ttu-id="d76ba-394">Можно задать несколько файлов ответов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-394">You can specify multiple response files.</span></span> <span data-ttu-id="d76ba-395">Однако, следует иметь в виду, что AzCopy не поддерживает вложенных фалов ответов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-395">However, note that AzCopy does not support nested response files.</span></span>

<span data-ttu-id="d76ba-396">**Применимо к** большим двоичным объектам, файлам, таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-396">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="y"></a><span data-ttu-id="d76ba-397">/Y</span><span class="sxs-lookup"><span data-stu-id="d76ba-397">/Y</span></span>
<span data-ttu-id="d76ba-398">Скрывает все запросы на подтверждение AzCopy.</span><span class="sxs-lookup"><span data-stu-id="d76ba-398">Suppresses all AzCopy confirmation prompts.</span></span>

<span data-ttu-id="d76ba-399">**Применимо к** большим двоичным объектам, файлам, таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-399">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="l"></a><span data-ttu-id="d76ba-400">/L</span><span class="sxs-lookup"><span data-stu-id="d76ba-400">/L</span></span>
<span data-ttu-id="d76ba-401">Задает только операцию перечисления, данные не копируются.</span><span class="sxs-lookup"><span data-stu-id="d76ba-401">Specifies a listing operation only; no data is copied.</span></span>

<span data-ttu-id="d76ba-402">AzCopy интерпретирует hello, используя для этого параметра моделирования для выполнения командной строки hello без этого параметра/l, а также счетчики, сколько объектов будут скопированы, можно указать параметр /V во же время toocheck объектов, которые копируются в подробный журнал hello hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-402">AzCopy interprets hello using of this option as a simulation for running hello command line without this option /L and counts how many objects are copied, you can specify option /V at hello same time toocheck which objects are copied in hello verbose log.</span></span>

<span data-ttu-id="d76ba-403">Hello поведение этого параметра также определяется наличие hello hello рекурсивные параметр /S и файл шаблона режим /Pattern и расположение hello hello исходных данных.</span><span class="sxs-lookup"><span data-stu-id="d76ba-403">hello behavior of this option is also determined by hello location of hello source data and hello presence of hello recursive mode option /S and file pattern option /Pattern.</span></span>

<span data-ttu-id="d76ba-404">При использовании этого параметра AzCopy необходимы права на ЧТЕНИЕ и СОЗДАНИЕ СПИСКА для местоположения исходных данных.</span><span class="sxs-lookup"><span data-stu-id="d76ba-404">AzCopy requires LIST and READ permission of this source location when using this option.</span></span>

<span data-ttu-id="d76ba-405">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-405">**Applicable to:** Blobs, Files</span></span>

### <a name="mt"></a><span data-ttu-id="d76ba-406">/MT</span><span class="sxs-lookup"><span data-stu-id="d76ba-406">/MT</span></span>
<span data-ttu-id="d76ba-407">Задает время последнего изменения hello загруженный файл toobe Здравствуйте таким же, как hello исходного большого двоичного объекта или файла.</span><span class="sxs-lookup"><span data-stu-id="d76ba-407">Sets hello downloaded file's last-modified time toobe hello same as hello source blob or file's.</span></span>

<span data-ttu-id="d76ba-408">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-408">**Applicable to:** Blobs, Files</span></span>

### <a name="xn"></a><span data-ttu-id="d76ba-409">/XN</span><span class="sxs-lookup"><span data-stu-id="d76ba-409">/XN</span></span>
<span data-ttu-id="d76ba-410">Исключает более новый исходный ресурс.</span><span class="sxs-lookup"><span data-stu-id="d76ba-410">Excludes a newer source resource.</span></span> <span data-ttu-id="d76ba-411">Hello ресурсов не копируются в случае hello время последнего изменения источника hello hello же или более новая, чем назначения.</span><span class="sxs-lookup"><span data-stu-id="d76ba-411">hello resource is not copied if hello last modified time of hello source is hello same or newer than destination.</span></span>

<span data-ttu-id="d76ba-412">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-412">**Applicable to:** Blobs, Files</span></span>

### <a name="xo"></a><span data-ttu-id="d76ba-413">/XO</span><span class="sxs-lookup"><span data-stu-id="d76ba-413">/XO</span></span>
<span data-ttu-id="d76ba-414">Исключает более старый исходный ресурс.</span><span class="sxs-lookup"><span data-stu-id="d76ba-414">Excludes an older source resource.</span></span> <span data-ttu-id="d76ba-415">Hello ресурсов не копируется в случае hello время последнего изменения источника hello hello же или более ранней, чем назначения.</span><span class="sxs-lookup"><span data-stu-id="d76ba-415">hello resource is not copied if hello last modified time of hello source is hello same or older than destination.</span></span>

<span data-ttu-id="d76ba-416">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-416">**Applicable to:** Blobs, Files</span></span>

### <a name="a"></a><span data-ttu-id="d76ba-417">/A</span><span class="sxs-lookup"><span data-stu-id="d76ba-417">/A</span></span>
<span data-ttu-id="d76ba-418">Отправляет только файлы, имеющие атрибут "архивный" hello набор.</span><span class="sxs-lookup"><span data-stu-id="d76ba-418">Uploads only files that have hello Archive attribute set.</span></span>

<span data-ttu-id="d76ba-419">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-419">**Applicable to:** Blobs, Files</span></span>

### <a name="iarashcnetoi"></a><span data-ttu-id="d76ba-420">/IA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="d76ba-420">/IA:[RASHCNETOI]</span></span>
<span data-ttu-id="d76ba-421">Передачи только файлы, содержащие любое hello указанный набор атрибутов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-421">Uploads only files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="d76ba-422">Доступные атрибуты:</span><span class="sxs-lookup"><span data-stu-id="d76ba-422">Available attributes include:</span></span>

* <span data-ttu-id="d76ba-423">R — файлы только для чтения</span><span class="sxs-lookup"><span data-stu-id="d76ba-423">R = Read-only files</span></span>
* <span data-ttu-id="d76ba-424">A — файлы, готовые к архивированию</span><span class="sxs-lookup"><span data-stu-id="d76ba-424">A = Files ready for archiving</span></span>
* <span data-ttu-id="d76ba-425">S — системные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-425">S = System files</span></span>
* <span data-ttu-id="d76ba-426">H — скрытые файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-426">H = Hidden files</span></span>
* <span data-ttu-id="d76ba-427">C — сжатые файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-427">C = Compressed files</span></span>
* <span data-ttu-id="d76ba-428">N — обычные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-428">N = Normal files</span></span>
* <span data-ttu-id="d76ba-429">E — зашифрованные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-429">E = Encrypted files</span></span>
* <span data-ttu-id="d76ba-430">T — временные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-430">T = Temporary files</span></span>
* <span data-ttu-id="d76ba-431">O — автономные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-431">O = Offline files</span></span>
* <span data-ttu-id="d76ba-432">I — неиндексированные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-432">I = Non-indexed files</span></span>

<span data-ttu-id="d76ba-433">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-433">**Applicable to:** Blobs, Files</span></span>

### <a name="xarashcnetoi"></a><span data-ttu-id="d76ba-434">/XA:[RASHCNETOI]</span><span class="sxs-lookup"><span data-stu-id="d76ba-434">/XA:[RASHCNETOI]</span></span>
<span data-ttu-id="d76ba-435">Исключает файлы, содержащие любое hello указанный набор атрибутов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-435">Excludes files that have any of hello specified attributes set.</span></span>

<span data-ttu-id="d76ba-436">Доступные атрибуты:</span><span class="sxs-lookup"><span data-stu-id="d76ba-436">Available attributes include:</span></span>

* <span data-ttu-id="d76ba-437">R — файлы только для чтения</span><span class="sxs-lookup"><span data-stu-id="d76ba-437">R = Read-only files</span></span>
* <span data-ttu-id="d76ba-438">A — файлы, готовые к архивированию</span><span class="sxs-lookup"><span data-stu-id="d76ba-438">A = Files ready for archiving</span></span>
* <span data-ttu-id="d76ba-439">S — системные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-439">S = System files</span></span>
* <span data-ttu-id="d76ba-440">H — скрытые файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-440">H = Hidden files</span></span>
* <span data-ttu-id="d76ba-441">C — сжатые файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-441">C = Compressed files</span></span>
* <span data-ttu-id="d76ba-442">N — обычные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-442">N = Normal files</span></span>
* <span data-ttu-id="d76ba-443">E — зашифрованные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-443">E = Encrypted files</span></span>
* <span data-ttu-id="d76ba-444">T — временные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-444">T = Temporary files</span></span>
* <span data-ttu-id="d76ba-445">O — автономные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-445">O = Offline files</span></span>
* <span data-ttu-id="d76ba-446">I — неиндексированные файлы</span><span class="sxs-lookup"><span data-stu-id="d76ba-446">I = Non-indexed files</span></span>

<span data-ttu-id="d76ba-447">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-447">**Applicable to:** Blobs, Files</span></span>

### <a name="delimiterdelimiter"></a><span data-ttu-id="d76ba-448">/Delimiter:"delimiter"</span><span class="sxs-lookup"><span data-stu-id="d76ba-448">/Delimiter:"delimiter"</span></span>
<span data-ttu-id="d76ba-449">Указывает, что символ-разделитель hello использовать toodelimit виртуальные каталоги в имени большого двоичного объекта.</span><span class="sxs-lookup"><span data-stu-id="d76ba-449">Indicates hello delimiter character used toodelimit virtual directories in a blob name.</span></span>

<span data-ttu-id="d76ba-450">По умолчанию использует AzCopy / как символ-разделитель hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-450">By default, AzCopy uses / as hello delimiter character.</span></span> <span data-ttu-id="d76ba-451">Однако, AzCopy поддерживает любой общий символ (например, @, # или %) в качестве разделителя.</span><span class="sxs-lookup"><span data-stu-id="d76ba-451">However, AzCopy supports using any common character (such as @, #, or %) as a delimiter.</span></span> <span data-ttu-id="d76ba-452">Если вам требуется один из этих специальных символов в командной строке hello tooinclude, заключите имя файла hello в двойные кавычки.</span><span class="sxs-lookup"><span data-stu-id="d76ba-452">If you need tooinclude one of these special characters on hello command line, enclose hello file name with double quotes.</span></span>

<span data-ttu-id="d76ba-453">Этот параметр применяется только для загрузки BLOB-объектов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-453">This option is only applicable for downloading blobs.</span></span>

<span data-ttu-id="d76ba-454">**Применимо к** большим двоичным объектам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-454">**Applicable to:** Blobs</span></span>

### <a name="ncnumber-of-concurrent-operations"></a><span data-ttu-id="d76ba-455">/NC:"number-of-concurrent-operations"</span><span class="sxs-lookup"><span data-stu-id="d76ba-455">/NC:"number-of-concurrent-operations"</span></span>
<span data-ttu-id="d76ba-456">Указывает номер hello одновременных операций.</span><span class="sxs-lookup"><span data-stu-id="d76ba-456">Specifies hello number of concurrent operations.</span></span>

<span data-ttu-id="d76ba-457">AzCopy по умолчанию запускается определенное число передачи пропускная способность hello tooincrease параллельных операций.</span><span class="sxs-lookup"><span data-stu-id="d76ba-457">AzCopy by default starts a certain number of concurrent operations tooincrease hello data transfer throughput.</span></span> <span data-ttu-id="d76ba-458">Обратите внимание, что большое количество параллельных операций в среде с низкой пропускной способностью может произойти переполнение hello сетевое подключение предотвращают hello операции полностью завершить.</span><span class="sxs-lookup"><span data-stu-id="d76ba-458">Note that large number of concurrent operations in a low-bandwidth environment may overwhelm hello network connection and prevent hello operations from fully completing.</span></span> <span data-ttu-id="d76ba-459">Регулируйте количество одновременных операций в зависимости от фактической доступной пропускной способности сети.</span><span class="sxs-lookup"><span data-stu-id="d76ba-459">Throttle concurrent operations based on actual available network bandwidth.</span></span>

<span data-ttu-id="d76ba-460">Hello верхний предел для одновременных операций — 512.</span><span class="sxs-lookup"><span data-stu-id="d76ba-460">hello upper limit for concurrent operations is 512.</span></span>

<span data-ttu-id="d76ba-461">**Применимо к** большим двоичным объектам, файлам, таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-461">**Applicable to:** Blobs, Files, Tables</span></span>

### <a name="sourcetypeblob--table"></a><span data-ttu-id="d76ba-462">/SourceType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="d76ba-462">/SourceType:"Blob" | "Table"</span></span>
<span data-ttu-id="d76ba-463">Указывает, что hello `source` ресурсом является большой двоичный объект в hello локальной среде разработки, работающее в эмуляторе хранилища hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-463">Specifies that hello `source` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="d76ba-464">**Применимо к** большим двоичным объектам и таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-464">**Applicable to:** Blobs, Tables</span></span>

### <a name="desttypeblob--table"></a><span data-ttu-id="d76ba-465">/DestType:"Blob" | "Table"</span><span class="sxs-lookup"><span data-stu-id="d76ba-465">/DestType:"Blob" | "Table"</span></span>
<span data-ttu-id="d76ba-466">Указывает, что hello `destination` ресурсом является большой двоичный объект в hello локальной среде разработки, работающее в эмуляторе хранилища hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-466">Specifies that hello `destination` resource is a blob available in hello local development environment, running in hello storage emulator.</span></span>

<span data-ttu-id="d76ba-467">**Применимо к** большим двоичным объектам и таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-467">**Applicable to:** Blobs, Tables</span></span>

### <a name="pkrskey1key2key3"></a><span data-ttu-id="d76ba-468">/PKRS:"key1#key2#key3#..."</span><span class="sxs-lookup"><span data-stu-id="d76ba-468">/PKRS:"key1#key2#key3#..."</span></span>
<span data-ttu-id="d76ba-469">Разбиений hello секции диапазона ключей tooenable Экспорт таблицы данных в параллельном режиме, что повышает скорость hello hello операции экспорта.</span><span class="sxs-lookup"><span data-stu-id="d76ba-469">Splits hello partition key range tooenable exporting table data in parallel, which increases hello speed of hello export operation.</span></span>

<span data-ttu-id="d76ba-470">Если этот параметр не указан, AzCopy использует один поток tooexport таблицы сущности.</span><span class="sxs-lookup"><span data-stu-id="d76ba-470">If this option is not specified, then AzCopy uses a single thread tooexport table entities.</span></span> <span data-ttu-id="d76ba-471">Например, если hello пользователь указывает /PKRS: «aa #bb», а затем AzCopy запускает три параллельных операций.</span><span class="sxs-lookup"><span data-stu-id="d76ba-471">For example, if hello user specifies /PKRS:"aa#bb", then AzCopy starts three concurrent operations.</span></span>

<span data-ttu-id="d76ba-472">Каждая операция экспортирует один из диапазонов ключей секций, как это показано на примере ниже:</span><span class="sxs-lookup"><span data-stu-id="d76ba-472">Each operation exports one of three partition key ranges, as shown below:</span></span>

  <span data-ttu-id="d76ba-473">[first-partition-key, aa)</span><span class="sxs-lookup"><span data-stu-id="d76ba-473">[first-partition-key, aa)</span></span>

  <span data-ttu-id="d76ba-474">[aa, bb)</span><span class="sxs-lookup"><span data-stu-id="d76ba-474">[aa, bb)</span></span>

  <span data-ttu-id="d76ba-475">[bb, last-partition-key]</span><span class="sxs-lookup"><span data-stu-id="d76ba-475">[bb, last-partition-key]</span></span>

<span data-ttu-id="d76ba-476">**Применимо к** таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-476">**Applicable to:** Tables</span></span>

### <a name="splitsizefile-size"></a><span data-ttu-id="d76ba-477">/SplitSize:"file-size"</span><span class="sxs-lookup"><span data-stu-id="d76ba-477">/SplitSize:"file-size"</span></span>
<span data-ttu-id="d76ba-478">Указывает Здравствуйте экспортированный файл разделить размер в МБ, hello допустимое минимальное значение — 32.</span><span class="sxs-lookup"><span data-stu-id="d76ba-478">Specifies hello exported file split size in MB, hello minimal value allowed is 32.</span></span>

<span data-ttu-id="d76ba-479">Если этот параметр не указан, AzCopy экспортирует tooa таблицы данных одного файла.</span><span class="sxs-lookup"><span data-stu-id="d76ba-479">If this option is not specified, AzCopy exports table data tooa single file.</span></span>

<span data-ttu-id="d76ba-480">Если hello таблица содержит данные BLOB-объект экспортированного tooa hello экспортированный файл достигает размера 200 hello ГБ для размера большого двоичного объекта, AzCopy разделяет hello экспортированного файла, даже если этот параметр не указан.</span><span class="sxs-lookup"><span data-stu-id="d76ba-480">If hello table data is exported tooa blob, and hello exported file size reaches hello 200 GB limit for blob size, then AzCopy splits hello exported file, even if this option is not specified.</span></span>

<span data-ttu-id="d76ba-481">**Применимо к** таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-481">**Applicable to:** Tables</span></span>

### <a name="entityoperationinsertorskip--insertormerge--insertorreplace"></a><span data-ttu-id="d76ba-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span><span class="sxs-lookup"><span data-stu-id="d76ba-482">/EntityOperation:"InsertOrSkip" | "InsertOrMerge" | "InsertOrReplace"</span></span>
<span data-ttu-id="d76ba-483">Задает поведение при импорте данных таблицы hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-483">Specifies hello table data import behavior.</span></span>

* <span data-ttu-id="d76ba-484">InsertOrSkip - пропускает существующую сущность или вставляет новую сущность, если она не существует в таблице hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-484">InsertOrSkip - Skips an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="d76ba-485">InsertOrMerge - объединяет существующую сущность или вставляет новую сущность, если она не существует в таблице hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-485">InsertOrMerge - Merges an existing entity or inserts a new entity if it does not exist in hello table.</span></span>
* <span data-ttu-id="d76ba-486">InsertOrReplace - заменяет существующую сущность или вставляет новую сущность, если она не существует в таблице hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-486">InsertOrReplace - Replaces an existing entity or inserts a new entity if it does not exist in hello table.</span></span>

<span data-ttu-id="d76ba-487">**Применимо к** таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-487">**Applicable to:** Tables</span></span>

### <a name="manifestmanifest-file"></a><span data-ttu-id="d76ba-488">/Manifest:"manifest-file"</span><span class="sxs-lookup"><span data-stu-id="d76ba-488">/Manifest:"manifest-file"</span></span>
<span data-ttu-id="d76ba-489">Указывает файл манифеста hello hello таблицы экспорта и импорта операции.</span><span class="sxs-lookup"><span data-stu-id="d76ba-489">Specifies hello manifest file for hello table export and import operation.</span></span>

<span data-ttu-id="d76ba-490">Этот параметр является необязательным, во время операции экспорта hello, AzCopy создает файл манифеста с предопределенным именем, если этот параметр не указан.</span><span class="sxs-lookup"><span data-stu-id="d76ba-490">This option is optional during hello export operation, AzCopy generates a manifest file with predefined name if this option is not specified.</span></span>

<span data-ttu-id="d76ba-491">Этот параметр является обязательным, во время операции импорта hello поиска файлов данных hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-491">This option is required during hello import operation for locating hello data files.</span></span>

<span data-ttu-id="d76ba-492">**Применимо к** таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-492">**Applicable to:** Tables</span></span>

### <a name="synccopy"></a><span data-ttu-id="d76ba-493">/SyncCopy</span><span class="sxs-lookup"><span data-stu-id="d76ba-493">/SyncCopy</span></span>
<span data-ttu-id="d76ba-494">Указывает ли toosynchronously копирование больших двоичных объектов или файлов между двумя конечными точками службы хранилища Azure.</span><span class="sxs-lookup"><span data-stu-id="d76ba-494">Indicates whether toosynchronously copy blobs or files between two Azure Storage endpoints.</span></span>

<span data-ttu-id="d76ba-495">По умолчанию AzCopy использует серверное асинхронное копирование.</span><span class="sxs-lookup"><span data-stu-id="d76ba-495">AzCopy by default uses server-side asynchronous copy.</span></span> <span data-ttu-id="d76ba-496">Укажите этот параметр tooperform синхронного копирования, который загружает большие двоичные объекты или файлы памяти toolocal и отправляет их tooAzure хранилища.</span><span class="sxs-lookup"><span data-stu-id="d76ba-496">Specify this option tooperform a synchronous copy, which downloads blobs or files toolocal memory and then uploads them tooAzure Storage.</span></span>

<span data-ttu-id="d76ba-497">Этот параметр можно использовать при копировании файлов в хранилище больших двоичных объектов в хранилище файлов или из большого двоичного объекта хранилища tooFile хранилища, и наоборот.</span><span class="sxs-lookup"><span data-stu-id="d76ba-497">You can use this option when copying files within Blob storage, within File storage, or from Blob storage tooFile storage or vice versa.</span></span>

<span data-ttu-id="d76ba-498">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-498">**Applicable to:** Blobs, Files</span></span>

### <a name="setcontenttypecontent-type"></a><span data-ttu-id="d76ba-499">/SetContentType:"content-type"</span><span class="sxs-lookup"><span data-stu-id="d76ba-499">/SetContentType:"content-type"</span></span>
<span data-ttu-id="d76ba-500">Задает тип содержимого MIME hello для целевых больших двоичных объектов или файлов.</span><span class="sxs-lookup"><span data-stu-id="d76ba-500">Specifies hello MIME content type for destination blobs or files.</span></span>

<span data-ttu-id="d76ba-501">Наборы AzCopy hello тип содержимого большого двоичного объекта или файла tooapplication/octet-stream по умолчанию.</span><span class="sxs-lookup"><span data-stu-id="d76ba-501">AzCopy sets hello content type for a blob or file tooapplication/octet-stream by default.</span></span> <span data-ttu-id="d76ba-502">Можно задать тип содержимого hello для всех больших двоичных объектов или файлы, явно указав значение для этого параметра.</span><span class="sxs-lookup"><span data-stu-id="d76ba-502">You can set hello content type for all blobs or files by explicitly specifying a value for this option.</span></span>

<span data-ttu-id="d76ba-503">При указании этого параметра без указания значения AzCopy задает каждый BLOB-объект или тип содержимого файла, в соответствии с tooits расширение файла.</span><span class="sxs-lookup"><span data-stu-id="d76ba-503">If you specify this option without a value, then AzCopy sets each blob or file's content type according tooits file extension.</span></span>

<span data-ttu-id="d76ba-504">**Применимо к** большим двоичным объектам и файлам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-504">**Applicable to:** Blobs, Files</span></span>

### <a name="payloadformatjson--csv"></a><span data-ttu-id="d76ba-505">/PayloadFormat:"JSON" | "CSV"</span><span class="sxs-lookup"><span data-stu-id="d76ba-505">/PayloadFormat:"JSON" | "CSV"</span></span>
<span data-ttu-id="d76ba-506">Задает формат hello hello таблицы экспортированного файла данных.</span><span class="sxs-lookup"><span data-stu-id="d76ba-506">Specifies hello format of hello table exported data file.</span></span>

<span data-ttu-id="d76ba-507">Если этот параметр не указан, по умолчанию AzCopy экспортирует файл данных таблицы в формате JSON.</span><span class="sxs-lookup"><span data-stu-id="d76ba-507">If this option is not specified, by default AzCopy exports table data file in JSON format.</span></span>

<span data-ttu-id="d76ba-508">**Применимо к** таблицам.</span><span class="sxs-lookup"><span data-stu-id="d76ba-508">**Applicable to:** Tables</span></span>

## <a name="known-issues-and-best-practices"></a><span data-ttu-id="d76ba-509">Известные проблемы и рекомендации</span><span class="sxs-lookup"><span data-stu-id="d76ba-509">Known Issues and Best Practices</span></span>
### <a name="limit-concurrent-writes-while-copying-data"></a><span data-ttu-id="d76ba-510">Ограничение одновременных операций записи при копировании данных</span><span class="sxs-lookup"><span data-stu-id="d76ba-510">Limit concurrent writes while copying data</span></span>
<span data-ttu-id="d76ba-511">При копировании больших двоичных объектов или файлов с помощью AzCopy, имейте в виду, возможно, изменение другое приложение hello данных при копировании его.</span><span class="sxs-lookup"><span data-stu-id="d76ba-511">When you copy blobs or files with AzCopy, keep in mind that another application may be modifying hello data while you are copying it.</span></span> <span data-ttu-id="d76ba-512">Если это возможно убедитесь, hello данных при копировании не изменяется во время операции копирования hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-512">If possible, ensure that hello data you are copying is not being modified during hello copy operation.</span></span> <span data-ttu-id="d76ba-513">Например при копировании виртуальный жесткий ДИСК, связанный с виртуальной машины Azure, убедитесь, что нет других приложений в настоящее время пишете toohello виртуального жесткого диска.</span><span class="sxs-lookup"><span data-stu-id="d76ba-513">For example, when copying a VHD associated with an Azure virtual machine, make sure that no other applications are currently writing toohello VHD.</span></span> <span data-ttu-id="d76ba-514">Toodo хорошим способом это по аренде toobe ресурсов hello копируются.</span><span class="sxs-lookup"><span data-stu-id="d76ba-514">A good way toodo this is by leasing hello resource toobe copied.</span></span> <span data-ttu-id="d76ba-515">Кроме того можно сначала создать моментальный снимок виртуального жесткого диска hello и скопируйте hello моментального снимка.</span><span class="sxs-lookup"><span data-stu-id="d76ba-515">Alternately, you can create a snapshot of hello VHD first and then copy hello snapshot.</span></span>

<span data-ttu-id="d76ba-516">Если не может помешать записи tooblobs или файлы, пока они копируются, то следует помнить, заданием hello hello время завершения работы других приложений, hello скопированных ресурсов может больше нет полной четности с ресурсами источника hello.</span><span class="sxs-lookup"><span data-stu-id="d76ba-516">If you cannot prevent other applications from writing tooblobs or files while they are being copied, then keep in mind that by hello time hello job finishes, hello copied resources may no longer have full parity with hello source resources.</span></span>

### <a name="run-one-azcopy-instance-on-one-machine"></a><span data-ttu-id="d76ba-517">Запускайте один экземпляр AzCopy на одном компьютере.</span><span class="sxs-lookup"><span data-stu-id="d76ba-517">Run one AzCopy instance on one machine.</span></span>
<span data-ttu-id="d76ba-518">AzCopy является использование hello спроектированный toomaximize скорость передачи данных ресурсов tooaccelerate машины hello, мы рекомендуем запустить только один экземпляр AzCopy на одном компьютере и укажите параметр hello `/NC` при необходимости несколько параллельных операций.</span><span class="sxs-lookup"><span data-stu-id="d76ba-518">AzCopy is designed toomaximize hello utilization of your machine resource tooaccelerate hello data transfer, we recommend you run only one AzCopy instance on one machine, and specify hello option `/NC` if you need more concurrent operations.</span></span> <span data-ttu-id="d76ba-519">Для получения дополнительных сведений введите `AzCopy /?:NC` hello командной строки.</span><span class="sxs-lookup"><span data-stu-id="d76ba-519">For more details, type `AzCopy /?:NC` at hello command line.</span></span>

### <a name="enable-fips-compliant-md5-algorithms-for-azcopy-when-you-use-fips-compliant-algorithms-for-encryption-hashing-and-signing"></a><span data-ttu-id="d76ba-520">Включайте FIPS-совместимые алгоритмы MD5 для AzCopy при использовании FIPS-совместимых алгоритмов для шифрования, хэширования и подписывания.</span><span class="sxs-lookup"><span data-stu-id="d76ba-520">Enable FIPS compliant MD5 algorithms for AzCopy when you "Use FIPS compliant algorithms for encryption, hashing and signing".</span></span>
<span data-ttu-id="d76ba-521">AzCopy по умолчанию использует .NET MD5 реализацию toocalculate hello MD5 при копировании объектов, но существуют некоторые требования безопасности, которым требуется параметром совместимости MD5 AzCopy tooenable FIPS.</span><span class="sxs-lookup"><span data-stu-id="d76ba-521">AzCopy by default uses .NET MD5 implementation toocalculate hello MD5 when copying objects, but there are some security requirements that need AzCopy tooenable FIPS compliant MD5 setting.</span></span>

<span data-ttu-id="d76ba-522">Для этого можно создать файл app.config `AzCopy.exe.config` со свойством `AzureStorageUseV1MD5` и поместить его в папку с файлом AzCopy.exe.</span><span class="sxs-lookup"><span data-stu-id="d76ba-522">You can create an app.config file `AzCopy.exe.config` with property `AzureStorageUseV1MD5` and put it aside with AzCopy.exe.</span></span>
...
<span data-ttu-id="d76ba-523">Для свойства «AzureStorageUseV1MD5» • True: значение по умолчанию hello, AzCopy использует реализацию .NET MD5.</span><span class="sxs-lookup"><span data-stu-id="d76ba-523">For property "AzureStorageUseV1MD5" • True - hello default value, AzCopy uses .NET MD5 implementation.</span></span>
<span data-ttu-id="d76ba-524">• False — AzCopy использует FIPS-совместимый алгоритм MD5.</span><span class="sxs-lookup"><span data-stu-id="d76ba-524">• False – AzCopy uses FIPS compliant MD5 algorithm.</span></span>

<span data-ttu-id="d76ba-525">Обратите внимание, что по умолчанию на компьютере Windows FIPS-совместимые алгоритмы отключены. Эту настройку можно изменить, набрав в окне запуска команд "secpol.msc" и выбрав "Настройки безопасности -> Локальные политики -> Параметры безопасности -> Системная криптография: Использовать FIPS-совместимые алгоритмы для шифрования, хеширования и подписывания".</span><span class="sxs-lookup"><span data-stu-id="d76ba-525">Note that FIPS compliant algorithms is disabled by default on your Windows machine, you can type secpol.msc in your Run window and check this switch at Security Setting->Local Policy->Security Options->System cryptography: Use FIPS compliant algorithms for encryption, hashing and signing.</span></span>

## <a name="next-steps"></a><span data-ttu-id="d76ba-526">Дальнейшие действия</span><span class="sxs-lookup"><span data-stu-id="d76ba-526">Next steps</span></span>
<span data-ttu-id="d76ba-527">Дополнительные сведения о хранилище Azure и AzCopy см. в разделе hello следующие ресурсы:</span><span class="sxs-lookup"><span data-stu-id="d76ba-527">For more information about Azure Storage and AzCopy, see hello following resources:</span></span>

### <a name="azure-storage-documentation"></a><span data-ttu-id="d76ba-528">Документация по хранилищу Azure:</span><span class="sxs-lookup"><span data-stu-id="d76ba-528">Azure Storage documentation:</span></span>
* [<span data-ttu-id="d76ba-529">Введение tooAzure хранилища</span><span class="sxs-lookup"><span data-stu-id="d76ba-529">Introduction tooAzure Storage</span></span>](../storage-introduction.md)
* [<span data-ttu-id="d76ba-530">Как toouse хранилища BLOB-объектов из .NET</span><span class="sxs-lookup"><span data-stu-id="d76ba-530">How toouse Blob storage from .NET</span></span>](../blobs/storage-dotnet-how-to-use-blobs.md)
* [<span data-ttu-id="d76ba-531">Как toouse хранения файлов из .NET</span><span class="sxs-lookup"><span data-stu-id="d76ba-531">How toouse File storage from .NET</span></span>](../storage-dotnet-how-to-use-files.md)
* [<span data-ttu-id="d76ba-532">Как toouse хранилище таблиц из .NET</span><span class="sxs-lookup"><span data-stu-id="d76ba-532">How toouse Table storage from .NET</span></span>](../../cosmos-db/table-storage-how-to-use-dotnet.md)
* [<span data-ttu-id="d76ba-533">Как управлять toocreate, или удалить учетную запись хранения</span><span class="sxs-lookup"><span data-stu-id="d76ba-533">How toocreate, manage, or delete a storage account</span></span>](../storage-create-storage-account.md)
* [<span data-ttu-id="d76ba-534">Перенос данных с помощью AzCopy для Linux</span><span class="sxs-lookup"><span data-stu-id="d76ba-534">Transfer data with AzCopy on Linux</span></span>](storage-use-azcopy-linux.md)

### <a name="azure-storage-blog-posts"></a><span data-ttu-id="d76ba-535">Записи блога по хранилищу Azure:</span><span class="sxs-lookup"><span data-stu-id="d76ba-535">Azure Storage blog posts:</span></span>
* [<span data-ttu-id="d76ba-536">Введение в предварительную версию библиотеки движения данных в хранилище Azure</span><span class="sxs-lookup"><span data-stu-id="d76ba-536">Introducing Azure Storage Data Movement Library Preview</span></span>](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/)
* [<span data-ttu-id="d76ba-537">AzCopy: введение в синхронное копирование и настраиваемый тип содержимого</span><span class="sxs-lookup"><span data-stu-id="d76ba-537">AzCopy: Introducing synchronous copy and customized content type</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2015/01/13/azcopy-introducing-synchronous-copy-and-customized-content-type.aspx)
* [<span data-ttu-id="d76ba-538">AzCopy: выпуск общедоступной версии AzCopy 3.0 и предварительной версии AzCopy 4.0 с поддержкой таблиц и файлов</span><span class="sxs-lookup"><span data-stu-id="d76ba-538">AzCopy: Announcing General Availability of AzCopy 3.0 plus preview release of AzCopy 4.0 with Table and File support</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/10/29/azcopy-announcing-general-availability-of-azcopy-3-0-plus-preview-release-of-azcopy-4-0-with-table-and-file-support.aspx)
* [<span data-ttu-id="d76ba-539">AzCopy: оптимизированные сценарии для крупномасштабного копирования</span><span class="sxs-lookup"><span data-stu-id="d76ba-539">AzCopy: Optimized for Large-Scale Copy Scenarios</span></span>](http://go.microsoft.com/fwlink/?LinkId=507682)
* [<span data-ttu-id="d76ba-540">AzCopy: поддержка геоизбыточного хранилища для доступа с правом чтения</span><span class="sxs-lookup"><span data-stu-id="d76ba-540">AzCopy: Support for read-access geo-redundant storage</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/04/07/azcopy-support-for-read-access-geo-redundant-account.aspx)
* [<span data-ttu-id="d76ba-541">AzCopy: передача данных с использованием перезапускаемого режима и маркера SAS</span><span class="sxs-lookup"><span data-stu-id="d76ba-541">AzCopy: Transfer data with re-startable mode and SAS token</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/09/07/azcopy-transfer-data-with-re-startable-mode-and-sas-token.aspx)
* [<span data-ttu-id="d76ba-542">AzCopy: использование копирования больших двоичных объектов между разными учетными записями</span><span class="sxs-lookup"><span data-stu-id="d76ba-542">AzCopy: Using cross-account Copy Blob</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/04/01/azcopy-using-cross-account-copy-blob.aspx)
* [<span data-ttu-id="d76ba-543">AzCopy: отправка и скачивание файлов для больших двоичных объектов Microsoft Azure</span><span class="sxs-lookup"><span data-stu-id="d76ba-543">AzCopy: Uploading/downloading files for Azure Blobs</span></span>](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/12/03/azcopy-uploading-downloading-files-for-windows-azure-blobs.aspx)
