...
Microsoft Azure — зафиксированных toohelping защиты и защиты вашего toomeet данных организации безопасности и соответствия обязательства.  Можно защитить данные хранятся в том toouse шифрование службы хранилища (SSE), автоматически шифрует данные при записи его toostorage, которое расшифровывает данные при его получении. Здравствуйте, шифрования и расшифровки автоматически, полностью прозрачно и 256 бит используется [шифрование AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard), один блок сильные hello шифры доступны.

Можно использовать ключи шифрования с SSE, управляемые корпорацией Майкрософт, или собственные ключи шифрования. В этой статье рассматриваются последний hello. Дополнительные сведения об использовании ключей, управляемых корпорацией Майкрософт, и о SSE в целом см. в статье [Шифрование службы хранилища Azure для неактивных данных (предварительная версия)](../storage-service-encryption.md).
...
Зачем toocreate собственные разделы? Он обеспечивает большую гибкость, позволяя toocreate, поворачивать, отключить и определить элементы управления доступом. Он также позволяет tooaudit hello шифрования ключей используется tooprotect данных.
...
tooparticipate в режиме предварительного просмотра hello, обратитесь к [ ssediscussions@microsoft.com ](mailto:ssediscussions@microsoft.com). Корпорация Майкрософт предоставляет tooparticipate специальной ссылкой в предварительной версии hello.

toolearn ссылаться более, toohello [часто задаваемые вопросы о](#frequently-asked-questions-about-storage-service-encryption-for-data-at-rest).
...
Можно включить для hello учетной записи хранилища с помощью hello SSE [портал Azure](https://portal.azure.com). В колонке параметров hello для учетной записи хранения hello найдите hello раздела службы BLOB-объектов как показано в hello рис. ниже, а затем щелкните шифрования.
...
На этом экране Если не отображается флажок «использовать свой собственный ключ» hello, вы не были утверждены для предварительного просмотра hello. Отправить сообщение электронной почты слишком[ ssediscussions@microsoft.com ](mailto:ssediscussions@microsoft.com) и запросить утверждение.
...
По умолчанию SSE использует ключи, управляемые корпорацией Майкрософт. toouse собственные разделы, установите флажок "hello". Затем можно либо указать свой ключ URI, или выберите ключ и хранилищем ключей из палитры hello.
...
Если вы хотите tootransfer данных в новую учетную запись хранения, чтобы он зашифрован, см. слишком[шаг 3 из Приступая к работе с шифрование службы хранилища для статических данных](https://docs.microsoft.com/en-us/azure/storage/storage-service-encryption#step-3-copy-data-to-storage-account).

## <a name="step-5-query-hello-status-of-hello-encrypted-data"></a>Шаг 5: Состояние запроса hello hello зашифрованных данных
состояние hello tooquery hello зашифрованных данных см. в слишком[шаг 4 из Приступая к работе с шифрование службы хранилища для статических данных](https://docs.microsoft.com/en-us/azure/storage/storage-service-encryption#step-4-query-the-status-of-the-encrypted-data).
...
О. Нет. При включении SSE при создании учетной записи хранилища hello, можно использовать только ключи Майкрософт для управляемого кода. При желании toouse управляется пользователем ключи необходимо обновить свойства учетной записи хранения hello. Можно использовать REST или один из tooprogrammatically библиотеки клиента хранения hello обновления учетной записи хранилища или обновить с помощью портала Azure hello после создания учетной записи hello свойства учетной записи хранения hello.

**Вопрос. Могу ли я отключить шифрование при использовании функции SSE с управляемыми клиентом ключами?**

Ответ. Нет, при использовании функции SSE с управляемыми клиентом ключами отключить шифрование невозможно. toodisable шифрования, необходимо переключиться ключи toousing Майкрософт для управляемого кода. Это можно сделать с помощью портала Azure hello или PowerShell.
...
Ответ. Функция SSE доступна только в определенных регионах для хранилища BLOB-объектов в рамках этой предварительной версии. По электронной почте [ ssediscussions@microsoft.com ](mailto:ssediscussions@microsoft.com) toocheck для обеспечения доступности и сведения о предварительной версии. 

**Вопрос. как связаться кого-либо если я все проблемы, так и tooprovide отзыв?**

Ответ контакта [ ssediscussions@microsoft.com ](mailto:ssediscussions@microsoft.com) для проблем, связанных tooStorage шифрования службы. 

## <a name="next-steps"></a>Дальнейшие действия

*   Дополнительные сведения о hello полный набор безопасности возможности, позволяющие разработчикам создавать безопасные приложения см. в разделе hello [руководство по безопасности хранилища](https://docs.microsoft.com/en-us/azure/storage/storage-security-guide).
