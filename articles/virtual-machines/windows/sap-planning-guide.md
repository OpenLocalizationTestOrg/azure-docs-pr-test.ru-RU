...
---
# <a name="sap-netweaver-on-azure-windows-virtual-machines-vms--planning-and-implementation-guide"></a>SAP NetWeaver на виртуальных машинах Windows в Azure. Руководство по планированию и внедрению
...
Hello документ дополняет hello документацию по установке SAP и примечаниях SAP, которые представляют Привет основные ресурсы для установки и развертывания программного обеспечения SAP в заданный платформы.
...
Корпорация Майкрософт предоставляет службы виртуальных машин Microsoft Azure как комплексную платформу «инфраструктура как услуга» (IaaS). Виртуальные машины Azure (IaaS) поддерживают приложения на основе SAP NetWeaver. В этом техническом документе описывается, как tooplan и реализуйте SAP NetWeaver на основе приложений в Microsoft Azure как платформу hello по выбору.

Hello документе будут рассмотрены два основных аспекта:

* Первая часть Hello описываются две модели развертывания для приложений на основе SAP NetWeaver в Azure. Также в ней описываются общие процессы управления Azure и их специфика для развертываний SAP.
* Вторая часть Hello предоставляются сведения о реализации hello два разных сценария, описанных в первой части hello.
...
В документе hello мы будем использовать hello следующими условиями:
...
Также ознакомьтесь с hello [SCN вики-сайте](https://wiki.scn.sap.com/wiki/display/HOME/SAPonLinuxNotes) , содержащий все примечания SAP для Linux.
...
* Здравствуйте, системы SAP (и VM(s)) представляют изолированный сценарий в Azure, который лишь требует открытого подключения к Интернету для конечных пользователей и не требует tooother подключение виртуальных машин в Azure.
...
* В случае, когда hello демонстрационную версию и сценариев обучения, реализованных на нескольких виртуальных машин, Active Directory / OpenLDAP/или служба DNS является обязательным для каждого набора виртуальных машин.
...
Этот сценарий является распределенным и допускает множество моделей развертывания. Это может быть просто описано, как при запуске некоторые части hello SAP альбомная локальной и другие части hello ландшафт SAP в Azure. Все аспекты hello факт, что часть компонентов SAP hello работает в Azure должны быть прозрачными для конечных пользователей. Поэтому Здравствуйте SAP транспорта Correction System (STMS), взаимодействие с RFC, печать, безопасность (например SSO), т. д. будут работать для hello систем SAP в Azure. Но hello межсетевых локальных ресурсах также описан сценарий, где работает hello завершения ландшафт SAP в Azure с доменом клиента hello и DNS расширяются до Azure.
...
>
> Причиной этих ограничений Hello — требование hello очень низкой задержкой сети высокую производительность в одной системе, особенно между экземплярами приложений hello и hello уровнем СУБД системы SAP.
>
>
...
размер Hello и ограничения различных размеров разных виртуальных машин предлагаемых можно увидеть в таблице [в данной статье][virtual-machines-sizes]

Как вы можете заметить, есть несколько семейств (серий) виртуальных машин. Начиная с декабря 2015 г. можно отличить hello следующие семейства виртуальных машин:
...
* Типы виртуальных машин серии D: производительность выше, чем у серии A0–A7. Не все типы ВМ hello сертифицированные с SAP.
* Типов ВМ серии DS: использовать же узлы как серии D, но будет может tooconnect tooAzure хранилище Premium (в главе [хранилища Azure Premium] [ planning-guide-3.3.2] этого документа). Также не все типы сертифицированы для использования с SAP.
* Типы виртуальных машин серии G: это типы виртуальных машин с большим объемом памяти.
* Типов ВМ серии GS: как серия G, но включая toouse параметр hello хранилища Azure Premium (в главе [хранилища Azure Premium] [ planning-guide-3.3.2] этого документа). При использовании виртуальных машин серии GS как серверы баз данных является обязательным toouse хранилище Premium для DB данных и файлы журнала транзакций
...
Обратите внимание на то, что не все другую серию виртуальных Машин могут предлагаться в каждом из них hello регионов Azure (для следующей главе регионы Azure см. в разделе). Также не забывайте, что не все виртуальные машины и не все серии виртуальных машин сертифицированы для использования с SAP.
...
Дополнительную информацию об использовании шаблонов ARM см. здесь:

* [Развертывание и управление виртуальными машинами с помощью шаблонов диспетчера ресурсов Azure и hello Azure CLI][virtual-machines-linux-cli-deploy-templates]
...
Другой интересной функцией является hello возможность toocreate образов из виртуальных машин, позволяющее tooprepare определенные репозитории, из которых являются tooquickly может развертывать экземпляры виртуальной машины, которые соответствуют вашим требованиям.
...
Понятие hello toounderstand группы доступности Azure и hello способом, группы доступности связаны tooFault и домены обновления, ознакомьтесь с [в данной статье][virtual-machines-manage-availability]
...
Виртуальные машины Microsoft Azure используют хранилища разных типов. При реализации SAP в службах виртуальных машин Azure очень важным toounderstand hello различия между двумя основными типами хранилища:
...
Выше инструкции применяются toohello типов ВМ, сертифицированные с SAP. для некоторых компонентов СУБД используйте подлежат Hello Серия виртуальных Машин с отличным операций ввода-ВЫВОДА и пропускной способности. См. в разделе hello [руководство по развертыванию СУБД] [ dbms-guide] для получения дополнительных сведений.
...
Он возможных tooupload существующий VHD из локальной или создать пустые контейнеры в Azure и присоединения этих toodeployed виртуальных машин. Такие виртуальные жесткие диски называются дисками Azure.
...
* Избыточное хранилище зоны, которой будут распределены между изображения hello трех разных центрах обработки данных в hello же регионе Azure.
* Уровень избыточности по умолчанию — географическую избыточность, которая асинхронно реплицирует hello содержимое в другой 3 изображения hello данных в другой регион Azure, которая размещается в hello геополитические совпадают.

См. также hello таблицу поверх данной статьи в отношении toohello избыточности разные параметры: <https://azure.microsoft.com/pricing/details/storage/>

Дополнительные сведения в отношении tooAzure хранилища можно найти здесь:
...
Хранилища Azure Стандартная большой двоичный объект был hello тип хранилища, доступный во время выпуска Azure IaaS. Для каждого виртуального жесткого диска применялись квоты на число операций ввода-вывода в секунду. Задержка возникла не hello же класс как SAN и NAS-устройства обычно развертывается для мощных систем SAP размещается локально. Тем не менее, hello оказался достаточно для сотен стандартное хранилище Azure систем SAP, в то же время развертывания в Azure.

Стандартная хранилища Azure взимается по hello фактические данные, хранящиеся, тома hello транзакции с хранилищем, исходящую передачу данных и избыточности с параметром, выбранным. Количество VHD, которые могут быть созданы на hello максимум 1 ТБ, но при условии, что эти оставаться пустым является бесплатной. Если затем вы указываете один виртуальный жесткий ДИСК с 100 ГБ, для хранения 100 ГБ, а не для hello номинальный размер приветствия создан VHD с плата.
...
Для этой цели много изменений были представлены какие hello две наиболее значимые являются:
...
В противоположной tooStandard хранилища, где не изменился, возможности зависит от размера hello hello диск (или VHD), хранилище Premium в настоящее время имеет три категории другой диск, которые приведены в конце hello в этой статье перед разделом hello вопросы и ответы: <https:/ /Azure.Microsoft.com/Pricing/Details/Storage/>

Вы видите, диск и операций ввода-ВЫВОДА или VHD-пропускной способности или VHD-зависят от категории размера hello hello дисков

Базовая себестоимость в случае hello хранилища Premium не hello фактические данные тома, хранящегося в такие виртуальные жесткие диски, но категорию размера hello таких виртуального жесткого диска, независимо от объема hello hello данных, хранящихся в hello виртуального жесткого диска.

Можно также создавать виртуальные жесткие диски в хранилище класса Premium, сопоставление не производится непосредственно на категории размер hello показано. Это может быть случай hello, особенно в том случае, если копирование виртуальных жестких дисков из стандартного хранилища в хранилище Premium. В таких случаях сопоставления toohello следующего наибольшего хранилище Premium диска параметра выполняется.

Имейте в виду, что только определенные Серия виртуальных Машин можно воспользоваться преимуществами hello хранилища Azure Premium. Начиная с декабря 2015 г. это hello и GS-серии DS. Hello серии DS является по сути hello же, как серии D с исключением hello, что серии DS имеет возможность hello toomount хранилище Premium на основе виртуальных машин Дополнительно tooVHDs, которые размещаются в стандартном хранилище Azure. Же является допустимым для серии tooGS серии G по сравнению.

Если вы извлекаете hello часть ВМ серии DS hello в [в этой статье] [ virtual-machines-sizes] вам также будет есть ограничения тома данных tooPremium хранения виртуальных жестких дисков на гранулярности hello hello уровня виртуальной Машины. Разные серии DS или виртуальные машины серии GS также имеют различные ограничения в отношении toohello количество виртуальных жестких дисков, которые могут быть подключены. Эти ограничения описаны в статье hello, упомянутых выше, а также. Но по сути это означает, если 32 tooa x P30 дисков и VHD, например подключения одной виртуальной Машины DS14 вы можете не получить 32 x hello максимальной пропускной способности диска P30. Вместо этого hello максимальной пропускной способности на уровне виртуальных Машин, как описано в статье hello будет ограничивать объем пропускной способности.
...
Дополнительные сведения об учетных записях хранения Azure см. в [этой статье][storage-scalability-targets]. Чтение в этой статье, вы получаете, существуют различия в ограничения hello стандартных учетных записей хранилища Azure и учетные записи хранения Premium. Основные различия — hello объем данных, которые могут быть сохранены в такой учетной записи хранилища. В стандартном хранилище hello том величиной больше, чем с хранилище Premium. На hello другие стороны hello стандартной учетной записи хранилища важности ограничен операций ввода-ВЫВОДА (см. столбец «Общая частота запросов»), а данное ограничение не имеет учетной записи хранилища Azure Premium hello. Мы рассмотрим сведения и результаты этих различий при обсуждении hello развертывания систем SAP, в особенности это касается серверов СУБД hello.

В учетной записи хранилища у вас hello вероятность toocreate различные контейнеры для hello цели организации и категоризация различных виртуальных жестких дисков. Эти контейнеры являются обычно используется tooe.g. отдельные VHD разных виртуальных машин. Производительность системы не зависит от того, сколько контейнеров находится в учетной записи службы хранилища Azure — один или несколько.
...
Microsoft Azure будет предоставлять сетевую инфраструктуру, позволяющую hello сопоставление все сценарии, которые мы хотим toorealize с программным обеспечением SAP. Ниже приведены возможности Hello.
...
Для сценариев между организациями, которые мы полагаемся на факт hello, hello локальной OpenLDAP/AD/DNS был расширен посредством VPN или частного подключения tooAzure. В некоторых сценариях, показанных здесь, может быть необходимые toohave реплику AD/OpenLDAP, установленная в Azure.
...
Hello MAC-адрес виртуального сетевого адаптера hello может изменяться например после изменить размер и hello Windows или Linux гостевой ОС будет окрашена hello новую сетевую карту и автоматически использовать DHCP tooassign hello адресов IP и DNS в этом случае.
...
Распределенное развертывание — это виртуальные машины Azure, подключенные к локальной сети с использованием прозрачного и постоянного VPN-подключения. Это ожидаемое toobecome hello наиболее SAP шаблон общего развертывания в Azure. Hello предполагается, что операционные процедуры и процессы с экземплярами SAP в Azure должны работать прозрачно. Это означает, что вы должны быть может tooprint из этих систем, а также использовать hello SAP Transport Management System (TMS) tootransport изменения из системы разработки в Azure tooa тестовую систему, развернутых локально. Дополнительную документацию по подключениям типа "сеть — сеть" можно найти в [этой статье][vpn-gateway-create-site-to-site-rm-powershell].

##### <a name="vpn-tunnel-device"></a>Устройство для VPN-туннеля
В заказ toocreate подключения сеть сеть (локального центра tooAzure данных центра обработки данных), вы должны будете tooeither получить и настроить VPN-устройство или использовать маршрутизации и удаленного доступа (RRAS) которая появилась как программный компонент с Windows Server 2012.
...
Точка сеть VPN требуется tooconnect машины каждого клиента с помощью свой собственный VPN в Azure. Для сценариев SAP hello, которую мы ищем подключение точка сайт не имеет смысла. Таким образом нет дальнейшие ссылки получит toopoint сеть VPN-подключение.
...
Мы должны toobe снимите о hello фактов, hello хранилища и сетевой инфраструктуры является общим для виртуальных машин, работающих в инфраструктуре Azure hello различных служб. И, как и центрах обработки данных клиента собственные hello избыточная Подготовка некоторые ресурсы инфраструктуры hello степень tooa месте. Hello платформе Microsoft Azure использует диска, ЦП, сети и других потребления ресурсов hello toolimit квоты и toopreserve постоянной и определенной производительности.  Hello разных типов ВМ (A5, A6, и т. д) имеют разные квоты hello количество дисков, ЦП, памяти и сети.

> [!NOTE]
> Ресурсы ЦП и памяти hello типов ВМ, поддерживаемых SAP, предварительно выделяются в узлах hello. Это означает, что после развертывания виртуальной Машины hello hello ресурсы на узле hello будут доступны в соответствии с определением hello типа ВМ.
>
>

Необходимо учитывать при планировании и определении размеров SAP в решениях Azure hello квоты для размера виртуальной машины.  описываются квот для VM Hello [здесь][virtual-machines-sizes].

Hello квоты представляют теоретические максимальные значения hello.  предел Hello операций на виртуальных жестких ДИСКОВ могут быть реализованы с небольшими операциями (8 КБ), но возможно не могут быть реализованы с большими операциями (1 МБ).  ограничение операций ввода-ВЫВОДА Hello устанавливается на hello детализации отдельных VHD.
...
Для новых разворачиваемых систем SAP следует проверены размеров, которая должна определить требования hello SAP к системе hello.
...
**Шаг 4**. Как описано [здесь][virtual-machines-sizes], Azure применяет квоту на количество операций ввода-вывода в секунду для каждого виртуального жесткого диска как в хранилище уровня "Стандартный", так и в хранилище уровня "Премиум". Зависит от типа ВМ hello, hello количество VHD, которые могут быть подключены зависит. В результате можно вычислить максимальное число операций ввода-ВЫВОДА, которую можно достичь с каждым из типов ВМ hello. Зависит от структуры файлов базы данных hello, вы можете создать чередующийся один том toobecome виртуальные жесткие диски в hello гостевой ОС. Однако если hello текущего операций ввода-ВЫВОДА тома в развернутой системе SAP превышает ограничения hello вычисляется hello крупного типа ВМ в Azure и не toocompensate вероятность с большим объемом памяти, рабочей нагрузки hello hello системы SAP может серьезно повлиять. В таких случаях можно нажать точки, где не следует развертывать hello системы в Azure.

**Шаг 5**: особенно в SAP систем, развернутых локально в двухуровневой конфигурации, hello скорее всего, что hello системы может потребоваться toobe, настроенный в Azure в трехуровневую конфигурацию. На этом шаге необходимо toocheck есть ли компонент в hello уровня приложения SAP которого нельзя масштабировать и который не может поместиться в hello ЦП и памяти, что ресурсы hello другое предложение типов ВМ Azure. Если на самом деле такой компонент, hello системы SAP и ее рабочей нагрузки нельзя развернуть в Azure. Но если вы можете масштабирования hello приложения SAP, компоненты в нескольких виртуальных машин Azure, hello системы может быть развернута в Azure.
...
Задачи администрирования и конфигурации для экземпляра виртуальной машины hello возможны в пределах hello портала Azure.
...
Hello портал Azure предоставляет базовые функциональные возможности toodeploy и настройка виртуальных машин и многих других служб Azure. Однако не вся доступная функциональность охватывается hello портала Azure. В hello портал Azure нет возможности tooperform задачи, такие как:
...
Более подробное описание шагов по как tooinstall, обновления и настройки hello командлетов Azure PowerShell можно найти также в [в этой главе hello руководство по развертыванию][deployment-guide-4.1].

Клиент до сих опыту, PowerShell (PS) безусловно является hello более мощные средства toodeploy виртуальных машин и toocreate пользовательские шаги в hello развертывания виртуальных машин. Toosupplement управления командлеты PS задачи, они в hello портала Azure или даже использовали командлеты PS всех клиентов hello работающих экземпляров SAP в Azure используются исключительно toomanage их развертывания в Azure. Поскольку специальные командлеты Azure папки hello hello же соглашение об именовании как hello более 2000 командлетов, очень простая задача для администраторов tooleverage Windows эти командлеты.
...
Развертывание расширения мониторинга Azure для SAP hello (в главе [решение мониторинга Azure для SAP] [ planning-guide-9.1] в этом документе) возможен только через PowerShell или интерфейс командной строки. Поэтому является обязательным toosetup и настроить PowerShell или интерфейс командной строки при развертывании или администрировании системы SAP NetWeaver в Azure.  

Как Azure предоставляет дополнительные функциональные возможности, новые командлеты PS реально добавленными toobe, что требует обновления командлетов hello. Таким образом он делает hello toocheck смысле веб-сайте загрузки Azure по крайней мере один раз в месяц hello <https://azure.microsoft.com/downloads/> для новой версии hello командлетов. Hello новая версия устанавливается только поверх старой версии hello.

Общий список команд PowerShell, связанных с Azure, см. здесь: <https://msdn.microsoft.com/library/azure/dn708514.aspx>.
...
Также ознакомьтесь с главе [Azure CLI для виртуальных машин Linux] [ deployment-guide-4.5.2] в hello [руководство по развертыванию] [ planning-guide] на как toodeploy toouse Azure CLI hello Azure Расширение мониторинга для SAP.

## <a name="different-ways-toodeploy-vms-for-sap-in-azure"></a>Различные способы toodeploy ВМ для SAP в Azure
В этой главе вы узнаете toodeploy различными способами hello виртуальной Машины в Azure. Также здесь рассматриваются дополнительные процедуры подготовки и процессы управления виртуальными жесткими дисками и виртуальными машинами в Azure.

### <a name="deployment-of-vms-for-sap"></a>Развертывание виртуальных машин для SAP
Microsoft Azure предлагает несколько способов toodeploy ВМ и связанные диски. Таким образом будет различия hello toounderstand очень важен, так как действия по подготовке виртуальных машин hello могут отличаться в зависимости от метода развертывания hello. Как правило мы будет взгляните на hello следующие сценарии:

#### <a name="4d175f1b-7353-4137-9d2f-817683c26e53"></a>Перемещение ВМ из локальной tooAzure с помощью специализированного диска
При планировании toomove определенную систему SAP из локальной tooAzure. Это можно сделать путем отправки VHD, содержащий hello ОС hello, hello двоичные файлы SAP и СУБД двоичные файлы, а также hello виртуальных жестких дисков с данными hello и файлы журналов для tooAzure СУБД hello. В отличие от слишком[сценария #2 ниже][planning-guide-5.1.2], оставьте имя узла hello, ИД безопасности SAP и учетные записи пользователей SAP в ВМ Azure hello, как они были настроены в локальной среде hello. Таким образом обобщение hello образ не является обязательным. См. в разделе главы [подготовки к перемещению ВМ из локальной tooAzure с помощью специализированного диска] [ planning-guide-5.2.1] этого документа в локальной среде действий по подготовке и отправке специализированного виртуальных машин или виртуальные жесткие диски tooAzure. Прочитайте раздел [сценарий 3: перемещение ВМ из локально с помощью специализированного VHD Azure с SAP] [ deployment-guide-3.4] в hello [руководство по развертыванию] [ deployment-guide]подробные инструкции по развертыванию такого образа в Azure.

#### <a name="e18f7839-c0e2-4385-b1e6-4538453a285c"></a>Развертывание виртуальной машины с помощью пользовательского образа
Из-за требований исправлениям toospecific вашей версии ОС или СУБД образы предоставленный hello в hello Azure Marketplace могут не соответствовать вашим потребностям. Таким образом может потребоваться toocreate ВМ с помощью собственных «private» образ ОС или СУБД ВМ, который можно разворачивать многократно. tooprepare такой «свой» образ для дублирования hello следующих элементов имеют считается toobe:
...
> ![Windows][Logo_Windows] Windows
>
> Параметры (такие как идентификатор безопасности Windows и имя узла) должен быть абстрагированные/универсальные на приветствия Windows Hello локальной виртуальной Машины с помощью команды sysprep hello.
>
>
> ![Linux][Logo_Linux] Linux
>
> Следуйте hello действия, описанные в следующих статьях для [SUSE] [ virtual-machines-linux-create-upload-vhd-suse] или [Red Hat] [ virtual-machines-linux-redhat-create-upload-vhd] tooprepare toobe виртуальный жесткий ДИСК отправлен tooAzure.
>
>

- - -
Если вы уже установили содержимое SAP в локальной виртуальной Машины (особенно для двухуровневых систем), hello параметры системы SAP можно адаптировать после развертывания hello hello виртуальной Машине Azure через экземпляр hello переименовать процедуру, поддерживаемых hello подготовки программного обеспечения SAP Диспетчер (Примечание SAP [1619720]). В разделе главы [подготовки для развертывания виртуальной Машины с помощью специального клиентского образа для SAP] [ planning-guide-5.2.2] и [Отправка VHD из локальной tooAzure] [ planning-guide-5.3.2]этого документа локальной действия по подготовке и процесса отправки обобщенный tooAzure виртуальной Машины. Прочитайте раздел [сценарий 2: развертывание ВМ с помощью пользовательского образа для SAP] [ deployment-guide-3.3] в hello [руководство по развертыванию] [ deployment-guide] подробное описание действий по развертывание такого образа в Azure.

#### <a name="deploying-a-vm-out-of-hello-azure-marketplace"></a>Развертывание ВМ из Azure Marketplace hello
Хотелось бы toouse Microsoft или сторонних производителей предоставленный образ ВМ из Azure Marketplace toodeploy hello ВМ. После развертывания ВМ в Azure вы выполните hello же рекомендации и средства tooinstall hello программное обеспечение SAP или СУБД как это делается в локальной среде. Более подробные описания развертывания, см. в разделе главы [сценария 1: развертывание ВМ из Azure Marketplace для SAP hello] [ deployment-guide-3.2] в hello [руководство по развертыванию] [ deployment-guide].
...
* Виртуальные жесткие диски, которые toohello подключенных виртуальных Машин и должен устанавливаться в Azure toohello toobe необходимость виртуальной Машины в фиксированном формате VHD также. Hello такое же ограничение на размер диска операционной системы hello применяется также дисков toodata. Виртуальные жесткие диски могут иметь размер не более 1 ТБ. Динамические виртуальные жесткие диски будут преобразованный toostatic виртуальных жестких дисков при передаче hello виртуального жесткого диска с командлеты PowerShell или интерфейс командной строки
...
> Убедитесь, что /mnt/resource не используется, и что все диски подключены через uuid. Убедитесь, что запись hello также отражает подключения на основе uuid hello для hello ОС диска.
>
>
...
Файлы на виртуальном жестком диске, содержащие обобщенную ОС, также хранятся в контейнерах учетных записей хранения Azure. Можно развернуть новую виртуальную Машину из такого образа виртуального жесткого диска, ссылаясь на hello виртуального жесткого диска как источник VHD в файлах шаблона развертывания, как описано в главе [сценарий 2: развертывание ВМ с помощью пользовательского образа для SAP] [ deployment-guide-3.3] из hello [Руководство по развертыванию][deployment-guide].
...
> ![Linux][Logo_Linux] Linux
>
> Убедитесь, что /mnt/resource не используется, и что все диски подключены через uuid. Для hello ОС диска убедитесь, что запись hello также отражает подключения на основе uuid hello.
>
>

- - -
* В такой шаблон можно также заранее установить графический интерфейс SAP (для администрирования и настройки).
* Другое программное обеспечение hello toorun необходимые виртуальные машины успешно в сценариях между организациями, которые могут быть установлены до тех пор, пока это программное обеспечение может работать с hello переименование hello виртуальной Машины.
...
> ![Windows][Logo_Windows] Windows
>
> Последний шаг Hello — toolog в tooa ВМ с учетной записью администратора. Откройте окно командной строки Windows с правами администратора. Go too...\windows\system32\sysprep и запустите программу sysprep.exe.
...
> Процедура hello tooperform виртуальная машина уже запущена в Azure, выполните hello действия, описанные в [в этой статье][virtual-machines-windows-capture-image].
>
> ![Linux][Logo_Linux] Linux
>
> [Как toocapture toouse виртуальных машин Linux в качестве шаблона диспетчера ресурсов][virtual-machines-linux-capture-image]
>
>
...
Третий альтернативный бы toouse различных инструментов с GUI сторонних производителей. Но перед этим убедитесь, что эти средства поддерживают страничные BLOB-объекты Azure. Для наших целей мы должны toouse Azure страничный большой двоичный объект хранения (Здесь описываются различия hello: <https://msdn.microsoft.com/library/windowsazure/ee691964.aspx>). Также hello средств, предоставляемых Azure очень эффективно сжимать ВМ hello и виртуальные жесткие диски, для которых требуется загрузить toobe. Это важно, поскольку эффективное сжатие сокращает время передачи hello (который зависит от все равно hello передачи ссылки toohello целевые Интернет с помещение локальной hello и региона развертывания Azure hello). Это справедливо предположить, что отправка ВМ или VHD из данных Azure США на основе toohello Европейского расположения центров займет больше времени, чем отправка hello же ВМ или VHD toohello европейских центрах обработки данных Azure.
...
* Подписка tooyour входа с *AzureRmAccount входа*
* Задать подписки hello контекста с *набор AzureRmContext* и параметр SubscriptionId или Имя_подписки - статье <https://msdn.microsoft.com/library/mt619263.aspx>
* Отправка hello VHD с *добавить AzureRmVhd* tooan учетной записи хранилища Azure — в разделе <https://msdn.microsoft.com/library/mt603554.aspx>
* Диск ОС hello набор toohello конфигурации виртуального жесткого диска новой виртуальной Машины с *AzureRmVMOSDisk набор* -в разделе <https://msdn.microsoft.com/library/mt603746.aspx>
* Создание новой виртуальной Машины из hello конфигурации виртуальной Машины с *New AzureRmVM* -в разделе <https://msdn.microsoft.com/library/mt603754.aspx>
* Добавьте диск данных tooa новой виртуальной Машины с *добавить AzureRmVMDataDisk* -в разделе <https://msdn.microsoft.com/library/mt603673.aspx>

**Интерфейс командной строки Azure**

* Переключите режим диспетчера ресурсов tooAzure с *arm режим конфигурации azure*
* Подписка tooyour входа с *входа azure*
* Выберите подписку с помощью команды *azure account set `<subscription name or id`>*
* Отправить hello VHD с *передачи больших двоичных объектов хранилища azure* -в разделе [hello с помощью Azure CLI со службой хранилища Azure][storage-azure-cli]
* Создание новой виртуальной Машины указание hello передать виртуальный жесткий ДИСК как диск ОС с *создания виртуальной машины azure* и параметр -d
* Добавьте диск данных tooa новой виртуальной Машины с *ВМ диск присоединения новый*

**Шаблон**

* Отправить VHD с помощью Powershell или Azure CLI hello
* Развертывание hello виртуальной Машины с помощью шаблона JSON, ссылающиеся на hello виртуального жесткого диска, как показано в [этот пример шаблона JSON](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/201-vm-from-specialized-vhd/azuredeploy.json).

#### <a name="deployment-of-a-vm-image"></a>Развертывание образа виртуальной машины
tooupload существующей ВМ или VHD из hello в локальной сети в порядке toouse ее как образ виртуальной Машины Azure ВМ или VHD должны toomeet hello требования, перечисленные в главе [подготовки для развертывания виртуальной Машины с помощью специального клиентского образа для SAP] [ planning-guide-5.2.2] этого документа.

* Используйте *sysprep* в Windows или *waagent-deprovision* на Linux toogeneralize ВМ - см [как toocapture виртуальной машины Windows в hello развертывания диспетчера ресурсов модели] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] или [как toocapture toouse виртуальных машин Linux в качестве шаблона диспетчера ресурсов][virtual-machines-linux-capture-image-capture]
* Подписка tooyour входа с *AzureRmAccount входа*
* Задать подписки hello контекста с *набор AzureRmContext* и параметр SubscriptionId или Имя_подписки - статье <https://msdn.microsoft.com/library/mt619263.aspx>
* Отправка hello VHD с *добавить AzureRmVhd* tooan учетной записи хранилища Azure — в разделе <https://msdn.microsoft.com/library/mt603554.aspx>
* Диск ОС hello набор toohello конфигурации виртуального жесткого диска новой виртуальной Машины с *набор AzureRmVMOSDisk - SourceImageUri - CreateOption fromImage* -. в разделе <https://msdn.microsoft.com/library/mt603746.aspx>
* Создание новой виртуальной Машины из hello конфигурации виртуальной Машины с *New AzureRmVM* -в разделе <https://msdn.microsoft.com/library/mt603754.aspx>

**Интерфейс командной строки Azure**

* Используйте *sysprep* в Windows или *waagent-deprovision* на Linux toogeneralize ВМ - см [как toocapture виртуальной машины Windows в hello развертывания диспетчера ресурсов модели] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] или [как toocapture toouse виртуальных машин Linux в качестве шаблона диспетчера ресурсов] [ virtual-machines-linux-capture-image-capture] для Linux
* Переключите режим диспетчера ресурсов tooAzure с *arm режим конфигурации azure*
* Подписка tooyour входа с *входа azure*
* Выберите подписку с помощью команды *azure account set `<subscription name or id`>*
* Отправить hello VHD с *передачи больших двоичных объектов хранилища azure* -в разделе [hello с помощью Azure CLI со службой хранилища Azure][storage-azure-cli]
* Создание новой виртуальной Машины указание hello передать виртуальный жесткий ДИСК как диск ОС с *создания виртуальной машины azure* и параметр -Q

**Шаблон**

* Используйте *sysprep* в Windows или *waagent-deprovision* на Linux toogeneralize ВМ - см [как toocapture виртуальной машины Windows в hello развертывания диспетчера ресурсов модели] [ virtual-machines-windows-capture-image-prepare-the-vm-for-image-capture] или [как toocapture toouse виртуальных машин Linux в качестве шаблона диспетчера ресурсов] [ virtual-machines-linux-capture-image-capture] для Linux
* Отправить VHD с помощью Powershell или Azure CLI hello
* Развертывание hello виртуальной Машины с помощью шаблона JSON, ссылающиеся на hello образа виртуального жесткого диска, как показано в [этот пример шаблона JSON](https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/101-vm-from-user-image/azuredeploy.json).

#### <a name="downloading-vhds-tooon-premises"></a>Загрузка tooon локальных виртуальных жестких дисков
...
### <a name="transferring-vms-and-vhds-within-azure"></a>Передача виртуальных машин и виртуальных жестких дисков в пределах Azure
#### <a name="copying-sap-systems-within-azure"></a>Копирование систем SAP в пределах Azure
Система SAP или даже выделенный сервер СУБД, поддерживающий уровень приложения SAP, могут состоять из нескольких VHD, которые содержат либо hello ОС с двоичными файлами hello или hello данные и файлы базы данных SAP hello журналов. Здравствуйте, функциональность Azure копирования VHD, ни hello функциональность Azure сохранения VHD toodisk имеет механизм синхронизации, который бы несколько виртуальных жестких дисков моментальных снимков синхронно. Таким образом, состояние hello hello скопировать или сохранить VHD, даже при их подключении к hello одной виртуальной Машины будет отличаться. Это означает, что в конкретную ситуацию получения разных данных и в виду Здравствуйте разных VHD, hello базы данных в конец hello hello будет несогласованной.

**Заключение: Порядок toocopy или сохранить VHD, которые являются частью конфигурации системы SAP требуется система SAP toostop hello и потребовать tooshut вниз hello развертывания ВМ. Затем можно скопировать и только набор hello загрузки VHD tooeither создать копию hello системы SAP в Azure или локально.**

Диски данных хранятся в виде VHD-файлы в учетную запись хранилища Azure и можно напрямую подключить tooa виртуальной машине или использоваться в качестве изображения. В этом случае hello виртуального жесткого диска — расположение скопированный tooanother, прежде чем beeing присоединить toohello виртуальной машины. Полное имя Hello hello VHD-файл в Azure должно быть уникальным в пределах Azure. Как упоминалось ранее, уже, hello имя имеет вид трехкомпонентным именем, выглядит следующим образом:

    http(s)://<storage account name>.blob.core.windows.net/<container name>/<vhd name>

##### <a name="powershell"></a>PowerShell
Можно использовать командлеты Azure PowerShell toocopy виртуального жесткого диска, как показано в [в этой статье][storage-powershell-guide-full-copy-vhd].

##### <a name="cli"></a>Интерфейс командной строки
Можно использовать Azure CLI toocopy виртуального жесткого диска, как показано в [в данной статье][storage-azure-cli-copy-blobs]
...
* Создать контекст учетной записи хранилища для учетной записи хранения hello источника с *New AzureStorageContext* -разделе <https://msdn.microsoft.com/library/dn806380.aspx>
* Создать контекст учетной записи хранилища для hello целевой учетной записи хранения с *New AzureStorageContext* -разделе <https://msdn.microsoft.com/library/dn806380.aspx>
...
#### <a name="4efec401-91e0-40c0-8e64-f2dceadff646"></a>Структура виртуальной машины и VHD для развертываний SAP
В идеале hello Обработка структуры hello виртуальной машины и связанных hello виртуальные жесткие диски должны быть очень простыми. Для локальных установок клиенты разработали много способов структурирования серверной установки.

* Один базовый VHD, содержащий hello операционной системы и все двоичные файлы hello объекта hello СУБД и SAP. С марта 2015 г. этого виртуального жесткого диска можно вверх too1TB размер вместо более ранней ограничения, которые ограничены его too127GB.
* Один или несколько виртуальных жестких дисков, которые содержит hello файл журнала СУБД базы данных SAP hello и файлов журналов hello hello временной области хранения СУБД (если hello СУБД это поддерживает). Если требования к операций ввода-ВЫВОДА журнала базы данных hello высоки, toostripe необходимо несколько виртуальных жестких дисков в порядке tooreach hello операций ввода-ВЫВОДА тома требуется.
* Количество VHD, содержащих один или два файла базы данных из базы данных SAP hello и hello временные файлы данных СУБД также (если hello СУБД это поддерживает).
...
- - -
Hello количество VHD, используемых для файлов данных СУБД hello и hello тип хранилища Azure, размещенных на этих виртуальных жестких дисков должен определяться hello IOPS требования и необходимые задержки hello. Точные квоты описаны в [этой статье][virtual-machines-sizes].
...
* Файлы данных toodifferent трафика операций ввода-ВЫВОДА не всегда hello же, поскольку существующие системы клиента может по-разному размера данных файлов, представляющих их баз данных SAP. В результате оказалось лучше использовать конфигурацию RAID через несколько виртуальных жестких дисков tooplace hello файлов данных, LUN вырезано из тех, toobe. Обнаружены ситуациях, особенно в случае стандартного хранилища Azure, где скорость операций ввода-ВЫВОДА попаданий hello квоты для одного VHD для журнала транзакций СУБД hello. В таких сценариях hello используемого хранилища Premium рекомендуется или в качестве альтернативы статистической обработки несколько стандартных хранения виртуальных жестких дисков с программным RAID.
...
Следует помнить, что hello VHD, содержащий hello ОС, и как мы рекомендуем hello двоичные файлы SAP и hello базы данных (базовой ВМ), больше не только too127GB. Он теперь может содержать до too1TB в размере. Это должно быть достаточно места tookeep все hello необходимый файл, включая, например журналы пакетных заданий SAP.
...
#### <a name="disk-handling"></a>Управление дисками
В большинстве случаев необходимо toocreate дополнительные диски в базы данных SAP hello toodeploy заказа в hello виртуальной Машины. Мы говорили о вопросах hello на количество виртуальных жестких дисков в главе [структура ВМ/VHD для развертываний SAP] [ planning-guide-5.5.1] этого документа. Hello Azure портал позволяет tooattach и отключать диски после развертывания базовой ВМ. Hello диски можно подключать и отключать, если hello виртуальная машина работает и запущены, а также при остановке. При подключении диска, hello портал Azure предлагает tooattach пустой диск или существующий диск, который на данный момент времени не присоединен tooanother виртуальной Машины.

**Примечание**: вложенные tooone виртуальной Машины виртуальных жестких дисков можно только в любой момент времени.

![Подключение и отключение дисков: служба хранилища Azure класса "Стандартный"][planning-guide-figure-1400]

Необходимо toodecide, следует ли использовать toocreate следует tooselect существующий виртуальный жесткий ДИСК, который был отправлен ранее и должен быть присоединен toohello виртуальной Машины, теперь или новый и пустой VHD (который будет создан в учетную запись хранения hello базовой ВМ находится в приветствия).
...
> ![Windows][Logo_Windows] Windows
>
> [Tooattach данных места на диске в hello портал Azure][virtual-machines-windows-attach-disk-portal]
>
> Если диски подключены, вы должны toolog в в hello ВМ tooopen hello диспетчера дисков Windows. Если automount не включена, согласно рекомендациям в главе [Установка параметра automount для присоединенных дисков][planning-guide-5.5.3], hello новый подключенный том должен toobe выполнены через Интернет и инициализирована.
>
> ![Linux][Logo_Linux] Linux
>
> Если диски подключены, нужно toolog в в hello виртуальной Машины и инициализировать диски hello, как описано в [в данной статье][virtual-machines-linux-how-to-attach-disk-how-to-initialize-a-new-data-disk-in-linux]
>
>
...
>
> * [Параметры командной строки служебной программы DiskPart](https://technet.microsoft.com/library/cc766465.aspx)
...
> * Hello последний шаг мастера hello, нужен toogive hello имя правила, а затем сохраните правило hello нажатием кнопки «Готово»
>
...
как описано в: <https://help.sap.com/saphelp_nwpi71/helpdata/en/47/c56a6938fb2d65e10000000a42189c/content.htm>
...
#### <a name="setup-network-for-communication-between-hello-different-vms"></a>Здравствуйте, Настройка сети для обмена данными между разными ВМ
...
### <a name="scenario-of-a-sap-landscape"></a>Сценарий ландшафта SAP
...
Hello сценарий, показанный выше описан сценарий, где hello в локальной среде AD и OpenLDAP и расширена tooAzure DNS. На стороне локальной hello определенный диапазон IP-адрес зарезервирован на подписку Azure. диапазон IP-адресов Hello будет назначен tooan виртуальной сети Azure на стороне Azure hello.
...
Трафик RFC между системами, которые являются локальными, так и в Azure должен toowork. toosetup соединения вызовите транзакцию SM59 в исходной системе которых требуется toodefine соединение RFC hello целевой системы. Конфигурация Hello — примерно toohello стандартной установки соединения RFC.
...
Как показано в hello блок-схема выше одной части hello решение по мониторингу для SAP размещается в hello образа виртуальной Машины Azure и Azure галереи расширений, который является глобально реплицируемым репозиторием, управляемым операциями Azure. Он несет hello hello совместные SAP/MS, занимающейся hello Azure реализацию SAP toowork с операциями Azure toopublish новых версий расширения мониторинга Azure для SAP hello. Это расширение мониторинга Azure для SAP будет использовать hello расширения Microsoft Azure Diagnostics (WAD) или диагностики Azure Linux (LAD) tooget hello необходимые сведения.
...
Hello данные вашей учетной записи хранилища Microsoft Azure всегда являются реплицированных tooensure устойчивость и высокого уровня доступности и следовании hello об уровне ОБСЛУЖИВАНИЯ хранилища Azure, даже при временных сбоях оборудования лицо hello

Так как хранилище Azure — поддержание 3 изображения hello данных по умолчанию, RAID5 или RAID1 на нескольких дисках Azure не обязательны.
...
Если вы решите не toouse функциональные возможности как Server отказоустойчивой кластеризации Windows (WSFC) или эквивалентные Linux (hello последний не поддерживается один еще в Azure в сочетании с по SAP), перезапустите ВМ Azure загруженные tooprotect системы SAP для планового и незапланированные простои hello Azure физического сервера инфраструктуры и базовой платформой Azure в целом.
...
Пример архитектуры системы SAP NetWeaver, в которой используется высокая доступность инфраструктуры Azure:
...
tooachieve полного SAP системы высокого уровня доступности, мы должны tooprotect все критические SAP системные компоненты, например избыточных серверов приложений SAP и уникальные компоненты (например единственная точка отказа), такие как экземпляра SAP (A) SCS и СУБД.
...
Количество доменов сбоя и обновления, которые можно использовать в группе доступности Azure в единице масштабирования Azure, не бесконечно. Это означает, что помещение число виртуальных машин в одной группе доступности рано или поздно фактически hello, более одной виртуальной Машины выходит за пределы hello же сбоя или обновление домена

Развертывание нескольких сервера приложений SAP экземпляров в их выделенные виртуальные машины, и при условии, что мы получили 5 доменов обновления, hello следующий рисунок появляется в конце hello. Hello фактическое максимальное число доменов сбоя и обновления в наборе доступности может измениться в будущих hello:
...
* Все виртуальные машины с экземпляром (A)SCS из одной системы SAP находятся в одной группе доступности. Внутри этих виртуальных машин настроить Windows Sever отказоустойчивого кластера (WSFC) tooprotect (A) SCS экземпляр.
...
Это не уровней приложения hello поддерживаемых toosplit сам или приложения hello и СУБД между локальной средой и Azure. Система SAP полностью развертываться либо локально, либо в среде Azure. Это также не поддерживается toohave некоторые из серверов приложений hello запуска в Azure в локальной среде и некоторые другие. Это hello, начальная точка hello-обсуждений. Также не поддерживаются компоненты СУБД hello toohave системы SAP и hello уровень сервера приложений SAP, развернутых в двух разных регионах Azure. Например:  СУБД в западной части США, а уровень приложений SAP в центральной части США). Не поддерживает такие конфигурации обусловлено чувствительность задержки hello hello архитектура SAP NetWeaver.
...
Зависимость от hello выбранной (2-уровневой или 3-уровневой) существует конфигурации SAP может быть toobackup необходимость. содержимое Hello hello самой ВМ, а также toohave резервная копия базы данных hello. Hello СУБД связанные резервные копии являются ожидаемый toobe выполняться средствами базы данных. Подробное описание hello различных баз данных, можно найти в [руководство по СУБД][dbms-guide]. На Здравствуйте другой стороны, hello данных SAP можно архивировать автономно (включая также содержимое базы данных hello) как описано в этом разделе или оперативно, как описано в следующем разделе hello.

автономное резервное копирование Hello по сути требуют выключение hello виртуальной Машины через портал Azure hello и копия hello базового диска ВМ и всех присоединенных toohello виртуальных жестких дисков виртуальной Машины. Это сохранит точка изображения времени hello ВМ и соответствующий диск. Рекомендуется toocopy hello «резервное копирование», в другую учетную запись хранилища Azure. Поэтому hello процедуры, описанные в главах [копирование дисков между учетными записями хранения Azure] [ planning-guide-5.4.2] применит этого документа.
Помимо hello завершения работы с помощью hello Azure портал один можно также сделать через Powershell или интерфейс командной строки как описанные здесь: <https://azure.microsoft.com/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>

Восстановление этого состояния состоит из hello удаление базовой ВМ, а также hello исходных дисков hello базовая ВМ и подключенных VHD, копирование назад hello сохранен VHD toohello исходной учетной записи хранения и затем повторного развертывания hello системы.
В этой статье приведен пример, в том, как tooscript этот процесс в Powershell: <http://www.westerndevs.com/azure-snapshots/>

Убедитесь, что tooinstall новая лицензия SAP с момента restoing резервной копии виртуальной Машины, как описано выше создает новый ключ оборудования.

### <a name="online-backup-of-an-sap-system"></a>Оперативное резервное копирование системы SAP
Выполнить резервное копирование СУБД hello специальными способами, как описано в hello [руководство по СУБД][dbms-guide].

Другие виртуальные машины в системе SAP hello резервное копирование с помощью функции резервного копирования виртуальной машины Azure. Резервное копирование виртуальной машины в Azure появилась в начале 2015 г. и в то же время является toobackup стандартный метод завершения виртуальную Машину в Azure. Служба архивации Azure хранятся резервные копии hello в Azure и позволяет восстановление виртуальной Машины, еще раз.
...
> По состоянию на январь 2016 г. функция резервного копирования ВМ Azure не поддерживает виртуальные машины, развернутые с помощью Azure Resource Manager.
>
> ![Windows][Logo_Windows] Windows
>
> Теоретически виртуальных машин, которые выполнения может быть резервное копирование баз данных также согласованного Если hello СУБД систем поддерживает hello Windows VSS (служба теневого копирования томов <https://msdn.microsoft.com/library/windows/desktop/bb968832 (v=vs.85).aspx > ) как например SQL Server.
> Но следует помнить, что восстановление состояния базы данных на определенный момент времени невозможно, если резервная копия была создана с помощью функции резервного копирования ВМ Azure. Таким образом рекомендуется tooperform резервные копии баз данных с функциональность СУБД, не полагаясь на резервное копирование виртуальных Машин Azure
>
> tooget знакомы с резервное копирование виртуальных машин Azure, начните здесь: <https://azure.microsoft.com/documentation/articles/backup-azure-vms/>.
>
> Другие возможные значения — toouse сочетания Microsoft Data Protection Manager, установленные в виртуальной Машине Azure и Azure Backup для резервного копирования и восстановления баз данных. Дополнительные сведения см. здесь: <https://azure.microsoft.com/documentation/articles/backup-azure-dpm-introduction/>.  
>
> ![Linux][Logo_Linux] Linux
>
> Нет эквивалент tooWindows VSS в Linux, не существует. Резервное копирование СУБД SAP должно выполняться средствами СУБД. Hello СУБД SAP резервное копирование должно выполняться с помощью функциональность СУБД. Hello файловой системы, включая hello SAP связанные данные могут сохраняться, например, при использовании tar-файл как описано здесь: <http://help.sap.com/saphelp_nw70ehp2/helpdata/en/d3/c0da3ccbb04d35b186041ba6ac301f/content.htm>
>
>
...
* Резервное копирование hello ВМ, содержащей глобальный каталог hello hello системы SAP и с ним все профили hello hello различных экземпляров, смысла и должно выполняться с помощью системы архивации данных Windows и tar например в Linux. Так как существуют различия между Windows Server 2008 (R2) и Windows Server 2012 (R2), что позволяет проще toobackup с помощью более поздней hello Windows Server, рекомендуется toorun Windows Server 2012 (R2) операционной системой Windows виртуальной машине.
