...
Несколько виртуальных машин могут создаваться в hello же облачной службе. toounderstand эту виртуальную машины сценарии, в разделе [как tooconnect виртуальных машин с помощью виртуальной сети или облачной службы](../classic/connect-vms.md#connect-vms-in-a-standalone-cloud-service). В этом сценарии клиент на виртуальной машине предпринимает tooconnect tooSQL сервера выполняется на другой виртуальной машины в hello же облачной службе.
...
> [!NOTE]
> При использовании этого метода toocommunicate с SQL Server, все данные, исходящие из hello центра обработки данных Azure — тема toonormal [Ценообразование на исходящую передачу данных](https://azure.microsoft.com/pricing/details/data-transfers/).
> 
> 
...
Виртуальные сети включите вы toojoin домена tooa виртуальных машинах Azure. Соединение tooa домена является только hello toouse способ проверки подлинности Windows с помощью SQL Server. Hello другие варианты подключения требуется проверка подлинности SQL с именами пользователей и пароли.

Если планируется tooconfigure среде домена и проверки подлинности Windows, необязательно общедоступную конечную точку hello tooconfigure или hello проверки подлинности SQL и имена входа. В этом сценарии можно подключиться tooyour экземпляр SQL Server, указав имя виртуальной Машины SQL Server hello в строке подключения hello. Следующий пример Hello предполагается, что была настроена проверка подлинности Windows и что hello пользователю был предоставлен экземпляр SQL Server toohello доступа.
...
Перед подключение toohello экземпляр SQL Server из другой виртуальной Машиной или hello Интернета, необходимо выполнить следующие задачи hello:
...
Это важные tooreview все hello рекомендации по обеспечению безопасности для SQL Server на виртуальной машине Azure. Дополнительные сведения см. в статье [Вопросы безопасности SQL Server на виртуальных машинах Azure](../sql/virtual-machines-windows-sql-security.md).
