...
* **Локально избыточное хранилище (LRS) класса Premium.** Хранилище Azure класса Premium обеспечивает поддержку высокопроизводительных дисков с низкой задержкой для виртуальных машин с интенсивной рабочей нагрузкой ввода-вывода. Предусматриваются три реплики данных в пределах одного и того же центра обработки данных Azure в том или ином регионе Azure. Копии будут располагаться в разных доменах сбоя и обновления (основные понятия описываются в [этом][planning-guide-3.2] разделе [руководства по планированию][planning-guide]). В случае выхода из строя реплики данных из-за сбоя узла хранилища или диска автоматически создается новая реплика.
* **Локально избыточное хранилище (LRS).** В этом случае предусматриваются 3 реплики данных в одном и том же центре обработки данных Azure в регионе Azure. Копии будут располагаться в разных доменах сбоя и обновления (основные понятия описываются в [этом][planning-guide-3.2] разделе [руководства по планированию][planning-guide]). В случае выхода из строя реплики данных из-за сбоя узла хранилища или диска автоматически создается новая реплика.
...
Дополнительные сведения о развертывании компонентов, передающих данные узлов в SAPOSCOL и SAPHostAgent, и об управлении жизненным циклом этих компонентов см. в [руководстве по развертыванию][deployment-guide].
...
[comment]: <> (Для виртуальных машин в системе SAP можно создавать резервные копии с помощью функции архивации виртуальных машин Azure. Функция резервного копирования виртуальных машин Azure появилась в начале 2015 г.; сейчас это стандартный способ создания полных резервных копий виртуальных машин в Azure. Служба архивации Azure хранит резервные копии в Azure и позволяет восстанавливать состояние виртуальных машин.)
[comment]: <> ( для виртуальных машин, на которых работают базы данных, можно также создавать согласованные резервные копии, если системы СУБД поддерживают службу теневого копирования томов VSS Windows (<https://msdn.microsoft.com/library/windows/desktop/bb968832.aspx>). В SQL Server эта служба поддерживается. Поэтому использование резервного копирования виртуальных машин Azure позволяет получить восстанавливаемую резервную копию базы данных SAP. При этом следует помнить, что восстановление баз данных до определенной точки во времени невозможно, если резервная копия была создана с помощью службы архивации виртуальных машин Azure. Поэтому мы рекомендуем создавать резервные копии баз данных средствами СУБД, а не полагаться на функцию архивации виртуальных машин Azure.)
[comment]: <> (Ознакомиться с архивацией виртуальных машин Azure можно здесь: https://azure.microsoft.com/documentation/articles/backup-azure-vms.)
...
[comment]: <> (Статья по-прежнему ссылается на ASM)
[comment]: <> (Перед чтением информации о различных конкретных технологиях обеспечения высокой доступности, пригодных для SQL Server в Azure, можно ознакомиться с документом, который содержит дополнительные сведения и указатели, [здесь][virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions].)
...
В случае определенных ERP или всех рабочих нагрузок BW имеет смысл, в отношении производительности, чтобы поддерживать tempdb устройства, кроме того, созданной базы данных tempdb (SWPM или вручную) на диске C:\. При отсутствии дополнительной базы данных tempdb рекомендуется ее создать (примечание к SAP [1752266]).
...
[comment]: <> (Ссылка указывает на ASM)
...
* Правильно настройте кэширование файлов для каждого большого двоичного объекта Azure. Настройки кэширования зависят от того, что будет храниться в большом двоичном объекте (тома (т. е. файлы) данных или журналов SAP MaxDB), а также от уровня хранилища Azure ("Стандартный" или "Премиум"), как описано в разделе [Кэширование для виртуальных машин и виртуальных жестких дисков][dbms-guide-2.1].
...
#### <a name="storage-configuration"></a>Конфигурация хранилища
Так как в основе SAP liveCache лежат технологии SAP MaxDB, то все рекомендации для службы хранилища Azure, приведенные для SAP MaxDB в разделе [Конфигурация хранилища][dbms-guide-8.4.1], относятся также и к SAP liveCache.
...
#### <a name="storage-configuration"></a>Конфигурация хранилища
При настройке SAP Content Server для хранения файлов в базе данных SAP MaxDB все рекомендации для службы хранилища Azure, приведенные для SAP MaxDB в разделе [Конфигурация хранилища][dbms-guide-8.4.1], относятся также и к серверу содержимого SAP.
...
#### <a name="storage-configuration"></a>Конфигурация хранилища
Все файлы базы данных должны храниться на виртуальных жестких дисках (VHD) с файловой системой NTFS. Эти виртуальные жестки диски должны быть подключены к виртуальной машине Azure и созданы на основе хранилища страничных BLOB-объектов Azure (<https://msdn.microsoft.com/library/azure/ee691964.aspx>).
Сетевые диски всех типов и удаленные общие ресурсы, включая следующие файловые службы Azure, **невозможно** использовать для хранения файлов базы данных.
