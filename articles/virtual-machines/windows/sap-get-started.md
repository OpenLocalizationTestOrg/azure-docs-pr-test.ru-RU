...
---
# <a name="using-sap-on-azure-windows-virtual-machines-vms"></a>Использование SAP на виртуальных машинах Windows в Azure
...
Выбирая Microsoft Azure в качестве партнеру готовности облака SAP, можно будет tooreliably выполнения критически важных рабочих нагрузок SAP на scaaleable, соответствующих требованиям и исключительную платформы.  Получение hello масштабируемости, возможности и экономия Azure. С hello развернут партнерство между корпорацией Майкрософт и SAP и можно запускать приложения SAP в сценариях разработки и тестирования и производства в azure, полностью поддерживаются. Из SAP NetWeaver tooSAP S4/HANA, Linux tooWindows, tooSQL SAP HANA, у нас есть вас. 

Корпорация Майкрософт предоставляет службы виртуальных машин Microsoft Azure и крупные экземпляры SAP HANA в Azure в рамках комплексной платформы IaaS (инфраструктура как услуга). Так как в Azure поддерживаются многочисленные решения SAP, этот документ по началу работы будет использоваться как оглавление для нашего текущего набора документов по SAP. При добавлении библиотеки документов tooour - дополнительные заголовки вы увидите их добавить здесь. 
...
| --- | --- | --- |
| SAP HANA Developer Edition (включая hello HANA клиентского программного обеспечения состоит из SQLODBC, ODBO-только для Windows, ODBC, драйверов JDBC, HANA studio, а база данных HANA) |Red Hat Enterprise Linux, SUSE Linux Enterprise |A7, A8 |
...
Microsoft Azure сертифицирован для следующих продуктов SAP, с полной поддержкой корпорации Майкрософт и SAP hello.

| Продукт SAP | Гостевая ОС | Реляционная СУБД | Типы виртуальных машин |
| --- | --- | --- | --- |
| ПО SAP Business Suite |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (только Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP Business All-in-One |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (только Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| Бизнес-аналитика SAP BusinessObjects |Windows |Недоступно |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
| SAP NetWeaver |Windows, SUSE Linux Enterprise, Red Hat Enterprise Linux |SQL Server, Oracle (только Windows), DB2, SAP ASE |A5 tooA11, D11 tooD14, DS11 tooDS14, GS1 tooGS5 |
...
Сводка: Этой статье описываются различные tooconsider действия при работе с SAP NetWeaver на виртуальных машинах Microsoft Azure SUSE Linux (ВМ). По состоянию на 19 мая 2016 года SAP NetWeaver официально поддерживается на виртуальных машинах SUSE Linux в Azure. Все сведения о версиях Linux, версиях ядра SAP и т. д. можно найти в примечании SAP 1928533 "SAP Applications on Azure: Supported Products and Azure VM types" (Приложения SAP в Azure: поддерживаемые продукты и типы виртуальных машин Azure).
...
## <a name="deploying-sap-ides-ehp7-sp3-for-sap-erp-60-on-microsoft-azure"></a>В статье описывается развертывание SAP IDES EHP7 SP3 для SAP ERP 6.0 в Microsoft Azure.
Заголовок: руководство по aaaQuickstart для ручной установки SAP HANA на виртуальных машинах Azure

Сводка: В этой статье описывается как SAP toodeploy выполняющейся в Microsoft Azure с SQL Server и ОС Windows через устройство библиотеки SAP облака 3.0 интегрированными средами разработки. 

Последнее обновление: сентябрь 2016 г.

[Это руководство можно найти здесь.](sap-cal-ides-erp6-ehp7-sp3-sql.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)

## <a name="3da0389e-708b-4e82-b2a2-e92f132df89c"></a>Планирование и реализация
Заголовок: aaaSAP NetWeaver на виртуальных машинах Azure (ВМ) — реализация руководство по планированию и
...
Заголовок: aaaSAP NetWeaver на виртуальных машинах Azure (ВМ) — руководство по развертыванию
...
Заголовок: aaaSAP NetWeaver на виртуальных машинах Azure (ВМ) — руководство по развертыванию СУБД
...
## <a name="63dab028-2c4f-4636-8f99-90bbb264eaba"></a>Руководство по развертыванию и обеспечению высокого уровня доступности
Заголовок: aaaSAP NetWeaver на виртуальных машинах Azure (ВМ) — высокий уровень доступности в руководстве по развертыванию

Сводка: В этом документе описывается, как можно защитить единой точкой сбоя компоненты, например SAP ASCS/SCS hello и СУБД SAP в Azure. Компоненты SAP ASCS/SCS hello, СУБД и приложения серверы являются необходимым элементом для функции hello систем SAP NetWeaver, например SAP NetWeaver ABAP, Java SAP NetWeaver, систем ABAP + Java SAP NetWeaver. Таким образом toobe помещены в функции высокой доступности требуется разместить toomake убедиться, что эти компоненты можно сохранять работоспособность при сбое сервера или виртуальной Машины как в конфигурациях кластера Windows для ОС и среды Hyper-V.
