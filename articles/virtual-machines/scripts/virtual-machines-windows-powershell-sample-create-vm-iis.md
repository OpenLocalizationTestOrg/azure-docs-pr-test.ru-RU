...
Этот скрипт создает виртуальную машину Azure под управлением Windows Server 2016, а затем использует tooinstall виртуальной машины Azure настраиваемое расширение скриптов hello IIS. После выполнения сценария hello, можно получить доступ к веб-сайт IIS по умолчанию hello на общедоступный IP-адрес hello hello виртуальной машины.
...
| [Set-AzureRmVMExtension](/powershell/module/azurerm.compute/set-azurermvmextension) | Добавьте виртуальную машину toohello расширения виртуальной Машины. В этом образце hello расширение пользовательского скрипта является используется tooinstall IIS. |
