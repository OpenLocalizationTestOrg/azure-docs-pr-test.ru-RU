...
Этот скрипт создает виртуальную машину и использует расширение пользовательского скрипта tooinstall WordPress для hello виртуальной машине Azure. После выполнения скрипта hello, можно получить доступ к hello WordPress веб-сайт конфигурации на `http://<public IP of VM>/wordpress`. 
...
| [Set-AzureRmVMExtension](/powershell/module/azurerm.compute/set-azurermvmextension) | Добавьте hello настраиваемое расширение скриптов toohello виртуального компьютера, который вызывает сценарий tooinstall WordPress. |
