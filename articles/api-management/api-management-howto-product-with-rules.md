...
Ограничения скорости и квоты настраиваются в редакторе политик. В этом руководстве добавляются две политики: [Limit call rate per subscription](https://msdn.microsoft.com/library/azure/dn894078.aspx#LimitCallRate) (Ограничение скорости вызова по подписке) и [Set usage quota per subscription](https://msdn.microsoft.com/library/azure/dn894078.aspx#SetUsageQuota) (Установка квоты использования по подписке). Эти политики должны применяться в области продукта.

В меню **Управление API** слева щелкните **Политики**. В списке **Продукт** выберите **Бесплатная пробная версия**.
...
Ограничение скорости и политики квот – это входящие политики, поэтому поместите курсор в раздел входящих.

![Редактор политики][api-management-policy-editor-inbound]

Прокрутите список политик и найдите запись политики **Limit call rate per subscription** (Ограничение скорости вызова по подписке).
...
Как видно во фрагменте, политика позволяет ограничивать число операций и API продукта. В этом руководстве мы не будем использовать эту возможность, поэтому удалите элементы **api** и **operation** из элемента **rate-limit**, чтобы остался только внешний элемент **rate-limit**, как показано в следующем примере.
...
Чтобы настроить политику **Set usage quota per subscription** (Установка квоты использования по подписке), поместите курсор под только что добавленным элементом **rate-limit** в элементе **inbound**, а затем найдите и щелкните стрелку слева от политики **Set usage quota per subscription** (Установка квоты использования по подписке).
...
Аналогично политике **Set usage quota per subscription** (Установка квоты использования по подписке), политика **Limit call rate per subscription** (Ограничение скорости вызова по подписке) позволяет настраивать ограничения для операций и API продукта. В этом руководстве мы не будем использовать эту возможность, поэтому удалите элементы **api** и **operation** из элемента **quota**, как показано в следующем примере.
