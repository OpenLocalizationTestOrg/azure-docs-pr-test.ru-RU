...
### <a name="install-hello-azure-iot-hub-sdk-on-edison"></a>Установите пакет SDK Azure IoT Hub hello на Edison
Установка hello Azure IoT Hub SDK на Edison при помощи hello следующую команду:

```bash
gulp install-tools
```

Эта задача может занять длительное время toocomplete, в зависимости от сетевого подключения. Он нуждается toobe запустить только один раз для одного Edison.
