...
Сначала необходимо извлечь экземпляр Наблюдателя за сетями. Переменная `$networkWatcher` передается в командлет `Start-AzureRmNetworkWatcherResourceTroubleshooting` на шаге 4.
