...
---
# <a name="how-does-hyper-v-replication-tooazure-work-in-site-recovery"></a>Как работает tooAzure репликации Hyper-V в Site Recovery?


В этой статье описывается hello компонентов и процессов, задействованных при репликации локальных виртуальных машин Hyper-V, с помощью hello tooAzure [Azure Site Recovery](site-recovery-overview.md) службы.

Служба Site Recovery может реплицировать виртуальные машины Hyper-V в кластерах Hyper-V и на изолированных узлах, которые управляются с помощью компонента System Center Virtual Machine Manager (VMM) или без него.
...
Существует несколько компонентов, участвующих при репликации виртуальных машин Hyper-V tooAzure.
...
**Узел Hyper-V** | Узлы и кластеры Hyper-V можно развернуть с помощью сервера VMM или без него. | Если нет ни одного сервера VMM, hello поставщика Site Recovery на которых установлен hello узла tooorchestrate репликации с Site Recovery через hello Интернета. Если сервер VMM, hello поставщик установлен, а не на узле hello.<br/><br/> агент служб восстановления Hello устанавливается на репликацию данных toohandle hello узла.<br/><br/> Обмен данными между hello поставщика и агента hello защищен и зашифрован. Реплицированные данные в службе хранилища Azure также шифруются.
**Виртуальные машины Hyper-V** | На сервере узла Hyper-V должна быть запущена хотя бы одна виртуальная машина. | Ничего не должен tooexplicitly, установленных на виртуальных машинах.
...
**На рис. 1: Репликация tooAzure узла Hyper-V**

![Компоненты](./media/site-recovery-components/arch-onprem-azure-hypervsite.png)

**Рисунок 2: Репликация tooAzure облаков в Hyper-V в VMM**
...
**Рисунок 3: Процесс репликации и восстановления для tooAzure репликации Hyper-V**
