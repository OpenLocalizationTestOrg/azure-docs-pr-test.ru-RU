...
---
# <a name="step-4-plan-networking-for-physical-server-replication-tooazure"></a>Шаг 4: Планирование сети для репликации tooAzure физического сервера

Это статье кратко рассматриваются вопросы планирования при репликации локальных tooAzure физических серверов, с помощью hello сети [Azure Site Recovery](site-recovery-overview.md) службы.
...
## <a name="connect-tooreplica-azure-vms"></a>Подключить виртуальные машины Azure tooreplica
...
- **Использовать другой IP-адрес**: можно выбрать toouse диапазоном IP-адресов для сети виртуальной Машины Azure реплицированных hello. В этом сценарии машины hello получает новый IP-адрес после отработки отказа и обновление DNS не требуется.
- **Используйте hello же IP-адрес**: может потребоваться toouse hello же диапазон IP-адресов, что основного локального узла, для hello сети Azure после отработки отказа. Сохранение hello одинаковые IP-адреса упрощает hello восстановления за счет уменьшения проблемы, связанные с сети после отработки отказа. Тем не менее при выполнении репликации tooAzure, необходимо будет tooupdate маршруты с нового расположения hello hello IP-адресов после отработки отказа.
...
Для банка Woodgrove toobe может tooreplicate его tooAzure серверы во время сохранения hello IP-адресов Вот какие hello компании toodo:
...
3. При настройке подключения hello сайт сайт, в hello Azure сети, может осуществлять маршрутизацию трафика toohello локально (локальная сеть) только в том случае, если диапазон IP-адресов hello отличается от диапазона IP-адресов hello в локальной среде.
    ...
    - Это происходит потому, что Azure не знает, что нет активных машин в подсети hello и подсети hello создается только для аварийного восстановления.
...
2. tooensure, hello IP-адреса для машины сохраняются после отработки отказа, в свойствах машины hello > **Настройка**, укажите hello же IP-адрес, который hello server используется локальная и нажмите кнопку **Сохранить**.
3. При переключении машины hello Azure Site Recovery будет назначать hello, предоставляемые tooit IP адрес.
