...
Чтобы определить, используется ли локальный кэш, перейдите на вкладку **Параметры приложения** службы приложений. Если локальный кэш используется, параметр `WEBSITE_LOCAL_CACHE_OPTION` имеет значение `Always`. Дополнительные сведения о локальном кэше службы приложений Azure см. в [этой статье](https://docs.microsoft.com/azure/app-service/app-service-local-cache).
