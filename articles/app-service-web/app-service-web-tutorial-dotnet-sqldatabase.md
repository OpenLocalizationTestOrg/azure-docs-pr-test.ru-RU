...
Выберите базу данных SQL `DotNetAppSqlDb`. В нижней части автоматически появится созданное ранее подключение.
...
Щелкните таблицу `Todoes` правой кнопкой мыши и выберите **Просмотреть данные**. 
...
Найдите элемент `<td>`, который содержит вспомогательные методы `Html.ActionLink()`. Добавьте следующий код Razor над этим элементом:
