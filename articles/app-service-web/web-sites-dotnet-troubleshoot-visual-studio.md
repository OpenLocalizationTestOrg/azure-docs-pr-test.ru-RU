...
В этом руководстве показано, как использовать инструменты Visual Studio, которые позволяют отлаживать работу веб-приложения в [службе приложений](http://go.microsoft.com/fwlink/?LinkId=529714) путем запуска веб-приложения удаленно в [режиме отладки](http://www.visualstudio.com/get-started/debug-your-app-vs.aspx) или путем просмотра журналов приложения и веб-сервера.
...
В учебнике предполагается, что вы используете Visual Studio 2015 или 2013. Если вы используете Visual Studio 2013, для веб-заданий требуется [обновление 4](http://go.microsoft.com/fwlink/?LinkID=510314) или более поздней версии.
...
3. Разверните группу ресурсов, которая содержит веб-приложение, созданное в разделе [Начало работы с Azure и ASP.NET][GetStarted], а затем щелкните узел этого веб-приложения правой кнопкой мыши и выберите **Просмотреть параметры**.
...
6. Из раскрывающегося списка **Профиль** выберите тот же профиль, который вы использовали в разделе [Начало работы с Azure и ASP.NET][GetStarted].
7. Щелкните вкладку **Параметры** и измените значение **Конфигурация** на **Отладка**, затем щелкните **Опубликовать**.

    ![Публикация в режиме отладки](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-publishdebug.png)
8. Когда завершится развертывание и откроется URL-адрес Azure вашего веб-приложения, закройте браузер.
9. В **обозревателе сервера** щелкните правой кнопкой мыши веб-приложение и выберите **Подключить отладчик**.
    ...
    **Примечание**. При возникновении сложностей с запуском отладчика попробуйте сделать это с помощью **Cloud Explorer**, а не **обозревателя сервера**.
...
В этом разделе демонстрируется удаленная отладка с использованием проекта и веб-приложения, созданного на этапе [Начало работы с пакетом SDK веб-заданий Azure](websites-dotnet-webjobs-sdk.md).
...
5. В раскрывающемся списке **Профиль** выберите тот же профиль, который вы использовали в учебнике [Приступая к работе с пакетом SDK для веб-заданий Azure](websites-dotnet-webjobs-sdk.md).
...
На вкладке **Журналы Azure** окна **Вывод** расположено несколько кнопок и текстовое поле.
...
1. Щелкните правой кнопкой мыши **Хранилище** в узле Azure и выберите пункт **Создать учетную запись хранения**.

![Учетная запись хранения](./media/web-sites-dotnet-troubleshoot-visual-studio/createstor.png)

1. В диалоговом окне **Создание учетной записи хранения** введите имя для учетной записи хранения.
...
2. В раскрывающемся списке **Регион или территориальная группа** укажите ближайший к вам регион.
...
5. В окне Visual Studio **Веб-приложение Azure** откройте вкладку **Журналы**, а затем щелкните **Настроить ведение журнала на портале управления**.
...
6. На вкладке **Настройка** классического портала прокрутите вниз до раздела диагностики приложения и установите переключатель **Ведение журнала приложения (хранилище таблиц)** в положение **Включено**.
7. Измените значение параметра **Уровень ведения журнала** на **Сведения**.
...
9. В поле **Управление табличным хранилищем для диагностики приложений** щелкните флажок, чтобы закрыть окно.
10. На вкладке **Настройка** классического портала нажмите кнопку **Сохранить**.
11. В окне браузера, в котором отображается веб-приложение приложения, щелкните **Home** (Главная), затем **About** (О программе) и **Contact** (Контактная информация).

     Ведение журнала сведений, полученных при просмотре веб-страниц, будут записаны в учетную запись хранения.
12. На вкладке **Журналы** окна **Веб-приложение Azure** в Visual Studio щелкните **Обновить** в разделе **Сводка по диагностике**.
     ...
     (При появлении ошибки "Таблица не найдена" убедитесь, что вы перешли к страницам, которые выполняют трассировку, после включения параметра **Ведение журнала приложения (хранилище)** и нажатия кнопки **Сохранить**.)
...
13. Щелкните **Просмотреть все журналы приложений**
     ...
     (Если возникает ошибка "последовательность не содержит элементов", откройте **обозреватель сервера**, разверните узел своей учетной записи хранения в разделе **Azure**, а затем щелкните правой кнопкой мыши **Tables** и выберите **Обновить**.)

     ![Журналы хранилища в табличном представлении](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-tracelogtableview.png)

     Это представление показывает дополнительные поля, которые не отображаются в других представлениях. Это представление также позволяет фильтровать журналы, используя для создания запроса специальный пользовательский интерфейс построителя запросов. Дополнительные сведения см. в разделе "Работа с ресурсами таблицы – фильтрация объектов" руководства [Обзор ресурсов хранилища с помощью обозревателя сервера](http://msdn.microsoft.com/library/ff683677.aspx).
14. Чтобы просмотреть подробные сведения по отдельной строке, дважды щелкните одну из строк.
...
4. На [портале Azure](https://portal.azure.com) в колонке **Параметры** вашего веб-приложения щелкните **Учетные данные развертывания**, а затем введите новые имя пользователя и пароль.
...
Дополнительные сведения об использовании режима отладки в Visual Studio см. в разделе MSDN [Отладка в Visual Studio](http://msdn.microsoft.com/library/vstudio/sc65sadd.aspx) и статье [Рекомендации по отладке в Visual Studio 2010](http://weblogs.asp.net/scottgu/archive/2010/08/18/debugging-tips-with-visual-studio-2010.aspx).
