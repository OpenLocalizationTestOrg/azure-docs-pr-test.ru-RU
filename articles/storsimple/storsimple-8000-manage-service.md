...
Управление устройствами StorSimple серии 8000 теперь осуществляется на портале Azure. При наличии существующей службы для управления устройствами StorSimple рекомендуется переместить ее на портал Azure. Классический портал Azure станет недоступен для службы диспетчера StorSimple после 30 сентября 2017 г.

Функция перехода на портал Azure доступна на разных этапах. Если вы хотите перенести диспетчер на портал Azure, изучили последствия этого решения (описаны в [рекомендациях по переходу](#considerations-for-transition)), но не видите соответствующую функцию, [отправьте запрос на поддержку](https://aka.ms/ss8000-cx-signup).
...
* На устройстве запущено обновление 3.0 или более поздней версии. Если устройство работает под управлением более старой версии, необходимо установить последние обновления. Дополнительные сведения см. в статье [Установка обновления 4 на устройство StorSimple](storsimple-8000-install-update-4.md). При использовании облачного устройства StorSimple (8010/8020) создайте новое облачное устройство с обновлением 4.0. 
...
* Существующие командлеты PowerShell для управления службами Azure (ASM) не поддерживаются. Обновите скрипты для управления устройствами через Azure Resource Manager.
...
1. Перейдите к службе диспетчера StorSimple на классическом портале.
...
> [!NOTE]
> До завершения смены ключей на портале Azure службы диспетчера StorSimple не могут выполняться никакие операции.
> 
> 
...
Эти действия необходимо выполнить в интерфейсе Windows PowerShell устройства StorSimple при наличии нескольких устройств, зарегистрированных в службе диспетчера StorSimple. Ключ, полученный на шаге 2, следует использовать для обновления остальных устройств StorSimple, зарегистрированных в службе диспетчера StorSimple.
