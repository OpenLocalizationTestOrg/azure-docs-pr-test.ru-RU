# Обзор
## [Что такое приложения логики?](logic-apps-what-are-logic-apps.md)
## [Сценарии B2B с пакетом интеграции Enterprise](logic-apps-enterprise-integration-overview.md)
## [Соединители](../connectors/apis-list.md)

# Приступая к работе
## [Создание приложения логики](logic-apps-create-a-logic-app.md)
## [Шаблоны для приложений логики](logic-apps-use-logic-app-templates.md)
## [Создание приложений логики на основе шаблонов](logic-apps-arm-provision.md)
## [Переместить MABS приложений toologic приложений](logic-apps-move-from-mabs.md)

# Практическое руководство
## Создание
### [Создание и развертывание приложений логики в Visual Studio](logic-apps-deploy-from-vs.md)
### [Добавление условий и запуск рабочих процессов](logic-apps-use-logic-app-features.md)
### [Добавление операторов switch](logic-apps-switch-case.md)
### [Добавление пользовательского кода с помощью Функций Azure](logic-apps-azure-functions.md)
### [Циклы, области действия и индивидуальная обработка данных](logic-apps-loops-and-scopes.md)
### [Пакетная обработка сообщений](logic-apps-batch-process-send-receive-messages.md)
### [Создание определений приложений логики](logic-apps-author-definitions.md)
### [Вызов, активация или вложение приложений логики](logic-apps-http-endpoint.md)
### [Создание настраиваемых API для приложений логики](logic-apps-create-api-app.md)
### [Вызов настраиваемых API для приложений логики](logic-apps-custom-hosted-api.md)
### [Обработка ошибок и исключений](logic-apps-exception-handling.md)
### [Обработка типов содержимого](logic-apps-content-type.md)
### [Защита приложений логики](logic-apps-securing-a-logic-app.md)
### [Устранение неполадок](logic-apps-diagnosing-failures.md)
### [Ограничения и конфигурация](logic-apps-limits-and-config.md)

## Сценарии B2B с пакетом интеграции Enterprise
### [Учетные записи интеграции](logic-apps-enterprise-integration-create-integration-account.md)
### [Партнеры](logic-apps-enterprise-integration-partners.md)
### [Соглашения](logic-apps-enterprise-integration-agreements.md)
### [Обработка B2B](logic-apps-enterprise-integration-b2b.md)
### [Обработка XML](logic-apps-enterprise-integration-xml.md)
### [Обработка неструктурированных файлов](logic-apps-enterprise-integration-flatfile.md)
### [Проверка XML](logic-apps-enterprise-integration-xml-validation.md)
### [Добавление схем для проверки XML](logic-apps-enterprise-integration-schemas.md)
### [Преобразование XML](logic-apps-enterprise-integration-transform.md)
### [Добавление карт для преобразования XML](logic-apps-enterprise-integration-maps.md)
### [Добавление сертификатов для безопасности B2B](logic-apps-enterprise-integration-certificates.md)
### [Хранение метаданных для артефактов](logic-apps-enterprise-integration-metadata.md)
### [Интеграция Enterprise с AS2](logic-apps-enterprise-integration-as2.md)
### [Кодирование AS2](logic-apps-enterprise-integration-as2-encode.md)
### [Декодирование AS2](logic-apps-enterprise-integration-as2-decode.md)
### [Интеграция Enterprise с EDIFACT](logic-apps-enterprise-integration-edifact.md)
### [Кодирование EDIFACT](logic-apps-enterprise-integration-edifact-encode.md)
### [Декодирование EDIFACT](logic-apps-enterprise-integration-edifact-decode.md)
### [Как toohandle EDIFACT документы с UNH2.5 сегмент](logic-apps-enterprise-integration-edifact_inputfile_unh2.5.md)
### [Интеграция Enterprise с X12](logic-apps-enterprise-integration-x12.md)
### [Кодирование X12](logic-apps-enterprise-integration-x12-encode.md)
### [Декодирование X12](logic-apps-enterprise-integration-x12-decode.md)
### [Аварийное восстановление](logic-apps-enterprise-integration-b2b-business-continuity.md)
### [Ошибки и решения](logic-apps-enterprise-integration-b2b-list-errors-solutions.md)

## Доступ к локальным данным
### [Подключение tooon локальные данные](logic-apps-gateway-connection.md)
### [Установка шлюза данных](logic-apps-gateway-install.md)

## Автоматизация и развертывание
### [Создание шаблонов для автоматического развертывания](logic-apps-create-deploy-template.md)
### [Публикация из Visual Studio](logic-apps-deploy-from-vs.md)

## Управление и мониторинг
### [Управление приложениями логики в Visual Studio](logic-apps-manage-from-vs.md)
### [Управление учетными записями интеграции](logic-apps-enterprise-integration-accounts.md)
### [Мониторинг состояния приложений логики](logic-apps-monitor-your-logic-apps.md)
### [Мониторинг состояния приложений логики в OMS с помощью Log Analytics](logic-apps-monitor-your-logic-apps-oms.md)
### [Мониторинг сообщений B2B](logic-apps-monitor-b2b-message.md)
#### [Отслеживание сообщений B2B в OMS](logic-apps-track-b2b-messages-omsportal.md)
#### [Запрашивание сообщений B2B в OMS](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md)
#### [Схема отслеживания AS2](logic-apps-track-integration-account-as2-tracking-schemas.md)
#### [Схема отслеживания X12](logic-apps-track-integration-account-x12-tracking-schema.md)
#### [Настраиваемая схема отслеживания](logic-apps-track-integration-account-custom-tracking-schema.md)

## Примеры, сценарии и пошаговые руководства
### [Обзор](logic-apps-examples-and-scenarios.md)
### [Создание панели мониторинга социальных сетей без сервера](logic-apps-scenario-social-serverless.md)
### [Вызов приложений логики с помощью Функций Azure](logic-apps-scenario-function-sb-trigger.md)
### [Добавление обработки ошибок и исключений](logic-apps-scenario-error-and-exception-handling.md)
### [Обработка B2B](logic-apps-enterprise-integration-b2b.md)

## Цены и выставление счетов
### [Цены](https://azure.microsoft.com/pricing/details/logic-apps/)
### [Измерение использования](logic-apps-pricing.md)

# Бессерверные приложения
## Обзор
### [Бессерверные приложения в Azure](logic-apps-serverless-overview.md)
## Начало работы
### [Создание бессерверного приложения в Visual Studio](logic-apps-serverless-get-started-vs.md)
## Сценарии и пошаговые руководства
### [Сбор отзывов клиентов](logic-apps-scenario-social-serverless.md)
### [Отправка автоматических сообщений электронной почты](https://channel9.msdn.com/Blogs/Microsoft-Integration/Build-and-Deploy-Serverless-Part-1)
### [Обработка заказов B2B и EDI](logic-apps-enterprise-integration-overview.md)

# Справочные материалы
## [Язык определения рабочего процесса](logic-apps-workflow-definition-language.md)
## [Действия и триггеры рабочего процесса](logic-apps-workflow-actions-triggers.md)
## [REST API](/rest/api/logic/)
## [PowerShell](/powershell/module/azurerm.logicapp)
## Соединители: подробные сведения
### [AS2](logic-apps-enterprise-integration-as2.md)
### [Хранилище BLOB-объектов Azure](../connectors/connectors-create-api-azureblobstorage.md)
### [Функции Azure](logic-apps-azure-functions.md)
### [Azure Logic Apps](logic-apps-http-endpoint.md)
### [База данных SQL Azure;](../connectors/connectors-create-api-sqlazure.md)
### [Настраиваемые API-интерфейсы и веб-приложения](logic-apps-create-api-app.md)
### [DB2](../connectors/connectors-create-api-db2.md)
### [Dynamics 365 CRM Online](../connectors/connectors-create-api-crmonline.md)
### [EDIFACT](logic-apps-enterprise-integration-edifact.md)
### [Концентраторы событий](../connectors/connectors-create-api-azure-event-hubs.md)
### [Файловая система](logic-apps-using-file-connector.md)
### [Неструктурированный файл](logic-apps-enterprise-integration-flatfile.md)
### [FTP](../connectors/connectors-create-api-ftp.md)
### [HTTP](../connectors/connectors-native-http.md)
### [HTTP + Swagger](../connectors/connectors-native-http-swagger.md)
### [webhook HTTP](../connectors/connectors-native-webhook.md)
### [Informix](../connectors/connectors-create-api-informix.md)
### [Учетная запись интеграции](logic-apps-enterprise-integration-metadata.md)
### [MQ](../connectors/connectors-create-api-mq.md)
### [Office 365 Outlook](../connectors/connectors-create-api-office365-outlook.md)
### [База данных Oracle](../connectors/connectors-create-api-oracledatabase.md)
### [Запросы и ответы](../connectors/connectors-native-reqres.md)
### [Salesforce](../connectors/connectors-create-api-salesforce.md)
### [SAP](logic-apps-using-sap-connector.md)
### [Расписание](../connectors/connectors-native-recurrence.md)
### [Служебная шина](../connectors/connectors-create-api-servicebus.md)
### [SharePoint Online](../connectors/connectors-create-api-sharepointonline.md)
### [SharePoint Server](../connectors/connectors-create-api-sharepointonline.md)
### [SQL Server](../connectors/connectors-create-api-sqlazure.md)
### [Преобразование XML](logic-apps-enterprise-integration-transform.md)
### [Twitter](../connectors/connectors-create-api-twitter.md)
### [X12](logic-apps-enterprise-integration-x12.md)
### [Проверка XML](logic-apps-enterprise-integration-xml-validation.md)

## [Соединители: полные сведения](https://docs.microsoft.com/connectors/)
## Журнал схем
### [Общедоступная версия](logic-apps-schema-2016-04-01.md)
### [Предварительная версия](logic-apps-schema-2015-08-01.md)

# Ресурсы
## [Стратегия развития Azure](https://azure.microsoft.com/roadmap/)
## [Форум MSDN](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azurelogicapps)
## [Калькулятор цен](https://azure.microsoft.com/pricing/calculator/)
## [Обновления службы](https://azure.microsoft.com/updates/?product=logic-apps)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-logic-apps)
