...
Шлюз работает как служба Windows. Как и любая другая служба Windows, служба шлюза может быть запущена и остановлена различными способами. Например, можно открыть командную строку с дополнительными разрешениями на компьютере, на котором выполняется шлюз, и выполнить одну из команд, приведенных ниже.
...
Локальный шлюз данных настроен для использования `NT SERVICE\PBIEgwService` в качестве учетных данных входа службы Windows. По умолчанию шлюзу предоставляется право "входа в качестве службы" для компьютера, на котором он установлен.
