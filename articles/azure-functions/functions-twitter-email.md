...
В этом руководстве показано, как анализировать мнения, выраженные в записях в Twitter, с помощью Функций, Logic Apps и Azure Cognitive Services. Функция, активируемая HTTP, классифицирует твиты на категории green, yellow или red на основе оценки мнения. При обнаружении недопустимого мнения отправляется электронное сообщение. 
...
3. Щелкните **Данные+аналитика** > **Cognitive Services**. Затем используйте параметры, указанные в таблице, примите условия и установите флажок **Закрепить на панели мониторинга**.
    ...
    | **Тип API.** | API анализа текста | API, используемый для анализа текста.  |
...
4. Нажмите кнопку **Создать**, чтобы создать учетную запись. После создания учетной записи щелкните новую учетную запись Cognitive Services, прикрепленную к панели мониторинга. 

5. В учетной записи щелкните **Ключи**, скопируйте значение **ключа 1** и сохраните его. Этот ключ используется для подключения приложения логики к учетной записи Cognitive Services. 
...
1. Разверните свое приложение-функцию, нажмите кнопку **+** рядом с пунктом **Функции**, а затем щелкните шаблон **HTTPTrigger**. Введите `CategorizeSentiment` в качестве **имени** функции и нажмите кнопку **Создать**.

    ![Колонка приложения-функции, добавление функции](media/functions-twitter-email/add_fun.png)

2. Замените содержимое файла run.csx следующим кодом и нажмите кнопку **Сохранить**:
...
3. Чтобы проверить функцию, щелкните **Тест** в правой области сверху. После этого откроется вкладка "Тест". В разделе **Текст запроса** введите значение `0.2`, а затем нажмите кнопку **Запуск**. В тексте ответа возвратится значение **RED**. 
...
4. Затем введите **имя**, например `TweetSentiment`, используйте параметры, указанные в таблице, примите условия и установите флажок **Закрепить на панели мониторинга**.
    ...
    | **Группа ресурсов** | myResourceGroup | API, используемый для анализа текста.  |
    | **Расположение** | Восток США | Выберите расположение рядом с вами. |
    | **Группа ресурсов** | myResourceGroup | Выберите ту же имеющуюся группу ресурсов, что и раньше.|

4. Щелкните **Создать**, чтобы создать приложение логики. После создания приложения щелкните новое приложение логики, прикрепленное к панели мониторинга. Затем в конструкторе Logic Apps прокрутите вниз и выберите шаблон **Пустое приложение логики**. 
...
2. Используйте параметры триггера Twitter, указанные в таблице. 
    ...
    | Настройка      |  Рекомендуемое значение   | Описание                                        |
    | ----------------- | ------------ | ------------- |
    | **Текст поискового запроса** | #Azure | Используйте популярный хэш-тег, который приведет к созданию новых твитов за выбранный интервал. Если при использовании уровня "Бесплатный" и выбранного хэш-тега создается слишком много твитов, вы можете быстро использовать все транзакции в учетной записи Cognitive Services. |
...
3. Введите имя подключения, например `MyCognitiveServicesConnection`, вставьте ключ для сохраненной учетной записи Cognitive Services и нажмите кнопку **Создать**.  
...
3. В разделе **If Yes, Do Nothing** (Если да, ничего не предпринимать) нажмите кнопку **Добавить действие**, найдите `outlook.com`, щелкните **Отправить электронное письмо** и выполните вход в учетную запись Outlook.com.
...
5. При обнаружении потенциально негативного мнения вы получите электронное сообщение. Если вы еще не получили его, вы можете изменить код функции таким образом, чтобы каждый раз возвращалась категория RED:
    ...
    > [!IMPORTANT]
    > Завершив работу с этим руководством, вам следует отключить приложение логики. Так вы не будете платить за выполнение и не используете все транзакции в учетной записи Cognitive Services.
...
Дополнительные сведения об Azure Logic Apps см. в [этой статье](../logic-apps/logic-apps-what-are-logic-apps.md).
