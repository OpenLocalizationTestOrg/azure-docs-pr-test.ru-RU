...
---
# <a name="rest-based-application-lifecycle-sample"></a>Пример жизненного цикла приложения на основе REST
В этом примере демонстрируется hello жизненный цикл приложений фабрики служб через вызовы REST API. Дополнительные сведения о жизненного цикла приложения hello Service Fabric. в разделе [жизненный цикл приложений фабрики служб](service-fabric-application-lifecycle.md).

В этом примере выполняет следующие hello.

* Подготавливает hello **WordCount 1.0.0** образец из пакета приложения WordCount hello в хранилище образов hello.
* Отображает hello список типов приложений, включая WordCount 1.0.0.
* Создает приложение hello WordCount как **fabric: / WordCount**.
* Отображает hello список приложений, включая fabric: / WordCount версии 1.0.0.
* Подготавливает версии hello 1.1.0 hello образца hello WordCount из **WordCountUpgrade** пакета приложения в хранилище образов hello.
* Отображает hello список типов приложений, включая обоих WordCount 1.0.0 и **WordCount 1.1.0**.
* Обновляет tooversion приложения WordCount hello 1.1.0.
* Отображает hello список приложений, который включает в себя WordCount версии 1.1.0, но больше не содержит версии 1.0.0 WordCount.
* Удаление приложения WordCount hello.
* Отображает hello список приложений, которое больше не содержит fabric: / WordCount.
* Отменяет подготовку версии hello 1.1.0 пример счетчика слов hello.
* Отображает список hello типы приложений, который включает в себя WordCount 1.0.0, но больше не содержит WordCount 1.1.0.
* Отменяет подготовку версии hello 1.0.0 пример счетчика слов hello.
* Отображает список hello типы приложений, которые больше не входят WordCount.

## <a name="prerequisites"></a>Предварительные требования
В этом образце используется hello [пример счетчика слов](http://aka.ms/servicefabricsamples) (найдено в hello **Приступая к работе** образцы). Пример счетчика слов Hello должны быть построены и два пакета приложений необходимо скопированный toohello хранилища образов.

| Папка | Описание |
| --- | --- |
| WordCount |Hello WordCount образца приложения. Hello **ApplicationManifest.xml** файл содержит **ApplicationTypeVersion = «1.0.0»**. |
| WordCountUpgrade |Hello WordCount образца приложения. необходимо изменить файл ApplicationManifest.xml Hello слишком**ApplicationTypeVersion = «1.1.0»** toooccur обновления приложения hello tooallow. |

toocreate hello пакетов приложений и скопируйте их в хранилище образов toohello, принимают hello следующие шаги:

1. Копировать **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** слишком**C:\Temp\WordCount**. Это создает пакет приложения WordCount hello.
2. Скопируйте C:\Temp\WordCount слишком**C:\Temp\WordCountUpgrade**. При этом создается hello **WordCountUpgrade приложения** пакета.
3. Откройте файл **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** в текстовом редакторе.
4. В hello **ApplicationManifest** элемент, изменение hello **ApplicationTypeVersion** атрибут слишком**«1.1.0»**.  Это обновляет hello номер версии приложения hello.
5. Сохраните файл ApplicationManifest.xml hello изменен.
6. Выполните следующий сценарий PowerShell с правами администратора hello хранилища образов toohello приложения hello toocopy:
...
По завершении сценария PowerShell hello это приложение является toorun готовности.

## <a name="example"></a>Пример
Следующий пример Hello демонстрирует жизненный цикл приложений фабрики служб hello.
