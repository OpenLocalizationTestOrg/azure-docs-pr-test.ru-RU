...
В этом пошаговом руководстве показано, как содержимое toocopy (BLOB) из одного актива в другую в порядке toohandle избыточность для потоковой передачи по требованию. Этот сценарий полезен, если требуется tooset вверх сеть доставки содержимого Azure toofail к между двумя центрами обработки данных, в случае сбоя в одном центре данных. В этом пошаговом руководстве используется hello Azure Media Services SDK, API REST служб мультимедиа Azure hello и hello пакет SDK хранилища Azure toodemonstrate hello следующие задачи:
...
3. Кодирование активов hello в нескольких битах скорость MP4-файлов. 
4. Создание доступного только для чтения указателя подписанного URL-адреса, Это необходимо для hello исходный активов toohave доступ для чтения toohello контейнер в учетной записи хранения hello, связанный с активом источника hello.
5. Получите имя контейнера hello hello исходный ресурс из локатора подписи общего доступа только для чтения hello, на предыдущем шаге hello. Это необходимо для копирования больших двоичных объектов между учетными записями хранения (описано ниже в разделе hello).
6. Создание указателя origin для hello актива, который был создан путем кодирования задача hello. 

Затем toohandle hello отработки отказа:

1. Настройка учетной записи для служб мультимедиа в "Центре обработки данных B".
2. Создайте пустой актив целевой в целевом hello учетная запись служб мультимедиа.
3. Создание доступного для записи указателя подписанного URL-адреса, Это необходимо для hello целевой пустой актив toohave доступ на запись toohello контейнер в hello целевой учетной записи хранения, связанный с hello целевого ресурса.
4. Используйте hello пакет SDK хранилища Azure toocopy BLOB-объектов (файлов ресурсов) между учетную запись хранилища источника hello в «Центре обработки данных A» и hello целевой учетной записи хранения в «B. центра данных» Эти учетные записи хранения связаны с активы hello интерес.
5. Свяжите BLOB-объектов (файлов ресурсов), скопированный toohello целевой контейнер больших двоичных объектов с hello целевого ресурса. 
6. Создание указателя origin для средства hello в «B центра данных» и укажите идентификатор локатора hello, созданный для средства hello в «Центре обработки данных а.»

Это дает hello потоковой передачи URL-адреса которых hello относительные пути URL-адреса hello hello же (только hello базовый URL-адреса отличаются). 

Затем toohandle любые сбои в работе, можно создать сеть доставки содержимого на основе этих локаторы источника. 

применить Hello следующие вопросы:

* Текущая версия пакета SDK служб мультимедиа Hello не поддерживает программным путем создания IAssetFile сведения, будет воспринимать актива с помощью файлов ресурсов. Вместо этого используйте API REST служб мультимедиа CreateFileInfos toodo hello это. 
* Зашифрованные активы хранилища (AssetCreationOptions.StorageEncrypted) не поддерживаются для репликации (так как ключ шифрования hello отличается в обе учетные записи служб мультимедиа). 
* Если вы хотите tootake преимуществами динамической упаковки, убедитесь, что hello потоковой передачи конечной точки, из которого нужно toostream содержимого находится в hello **под управлением** состояния.

> [!NOTE]
> Рассмотрите возможность использования служб мультимедиа hello [средство Replicator](http://replicator.codeplex.com/) как альтернативный tooimplementing a отработки отказа, сценарий потоковой передачи вручную. Это средство позволяет tooreplicate активы по две учетные записи служб мультимедиа.
> 
> 

## <a name="prerequisites"></a>Предварительные требования
* Две учетные записи служб мультимедиа в новой или существующей подписке Azure. В разделе [как учетную запись служб мультимедиа tooCreate](media-services-portal-create-account.md).
...
1. С помощью Visual Studio toocreate новое решение, содержащее проект консольного приложения C# hello. Введите **HandleRedundancyForOnDemandStreaming** hello имя, а затем нажмите кнопку **ОК**.
2. Создать hello **SupportFiles** папку на hello же уровня как hello **HandleRedundancyForOnDemandStreaming.csproj** файл проекта. В разделе hello **SupportFiles** папки, создать hello **OutputFiles** и **MP4Files** папки. Скопируйте MP4-файл в hello **MP4Files** папки. (В этом примере hello **BigBuckBunny.mp4** файл используется.) 
3. Используйте **Nuget** tooMedia служб, связанных с tooDLLs tooadd ссылки. В **главном меню Visual Studio** выберите **Инструменты** > **Library Package Manager** (Диспетчер пакетов библиотеки) > **Консоль диспетчера пакетов**. В окне консоли hello, введите **windowsazure.mediaservices Install-Package**, и нажмите клавишу ВВОД.
4. Добавьте другие ссылки, необходимые для этого проекта: System.Configuration, System.Runtime.Serialization и System.Web.
5. Замените **с помощью** инструкций, которые были добавлены toohello **Programs.cs** файла по умолчанию hello следующие объекты:
...
6. Добавить hello **appSettings** toohello раздел **.config** файла и обновлять значения hello на основе Media Services и хранилища ключей и имя значения. 
...
В этом разделе создайте hello возможность toohandle избыточности.

1. Добавьте следующий класс Program toohello поля уровня класса hello.
...
2. Замените определение метода Main по умолчанию hello одном hello. Определения методов, вызываемых из метода Main, описаны ниже.
...
3. Hello после определения метода вызываются из метода Main.

    >[!NOTE]
    >Действует ограничение в 1 000 000 записей для разных политик служб мультимедиа (например, для политики Locator или ContentKeyAuthorizationPolicy). Следует использовать hello же идентификатор политики, если вы используете всегда hello же дней и доступа разрешения. Например используйте hello таким же ИДЕНТИФИКАТОРОМ для политик указатели, которые являются предполагаемого tooremain на месте в течение длительного времени (без передачи политики). Дополнительные сведения см. в [этом разделе](media-services-dotnet-manage-entities.md#limit-access-policies).
...
Теперь использовать запросов tooroute диспетчера трафика, между двумя центрами обработки данных hello и таким образом отработки отказа в случае любой сбоев.
