...
Этот учебник поможет выполнить шаги hello реализации базовой службы доставки содержимого видео по запросу (VoD) с приложением служб мультимедиа Azure (AMS), с помощью Java.
...
* Hello библиотек Azure для Java, которую можно установить из hello [центра разработчиков Java Azure][Azure Java Developer Center].
...
Hello следующий код показывает, как toocreate актива, передача toohello файла мультимедиа, выполнить задание с активом hello tootransform задач и создать toostream локатора видео.

Прежде чем использовать этот код необходимо tooset учетную запись служб мультимедиа. Сведения о настройке учетной записи см. в разделе [как учетную запись служб мультимедиа tooCreate](media-services-portal-create-account.md).

Подставьте собственные значения переменных «clientId» и «clientSecret» hello. Привет код также основан на локально хранящихся файлов. Вам потребуется tooprovide toouse собственный файл.
...
Документацию Javadoc по службам мультимедиа см. в [документации по библиотекам Azure для Java][Azure Libraries for Java documentation].
