...
Перейдите на вкладку **Интеграция** и введите для параметра "Таймер" понятное имя, которое будет использоваться в функции. В приведенном выше коде ожидается, что таймеру будет присвоено имя *myTimer*. Укажите [выражение CRON](../app-service-web/web-sites-create-web-jobs.md#CreateScheduledCRON) для таймера, который инициирует запуск функции каждую минуту, следующим образом: 0 \* \* \* \* \*.
