...
Шлюз приложений позволяет toohave веб-приложение Azure или другой несколькими клиентами службы как член пула серверной части. В этой статье tooyou о tooconfigure веб-приложение Azure со шлюзом приложений. Hello в первом примере показано, как tooconfigure существующие toouse шлюза приложения как член пула серверной части веб-приложения. Hello во втором примере показано, как toocreate новый шлюз приложения с веб-приложения, как член пула серверной части.
...
Hello пример добавления веб-приложения как внутреннем пуле член tooan существующие приложения шлюз. Оба hello коммутатора `-PickHostNamefromBackendHttpSettings`hello проверки конфигурации и `-PickHostNameFromBackendAddress` на серверной стороне http hello параметры должны задаваться в порядке для toowork приложения web.
...
Этот сценарий выполняет развертывание веб-приложения с asp.net hello по началу работы веб-сайта и шлюз приложений.
...
Узнайте, как tooconfigure перенаправление по следующему адресу: [Настройка перенаправления на использование шлюза приложений с помощью PowerShell](application-gateway-configure-redirect-powershell.md).
