...
Узнайте, как политики версий tooconfigure SSL и шифра наборов на шлюз приложений. Можно воспользоваться [списком стандартных политик](#predefined-ssl-policies), в котором содержатся различные конфигурации версий политики SSL и разрешенные комплекты шифров. Также имеется возможность toodefine hello [настраиваемой политики SSL](#configure-a-custom-ssl-policy) на основе требований.

## <a name="get-available-ssl-options"></a>Получение доступных параметров SSL

Hello `Get-AzureRMApplicationGatewayAvailableSslOptions` командлет предоставляет список доступных предопределенных политик, доступных комплектов шифров и версии протокола, которые могут быть настроены. Hello примере показан пример выходных данных из командлета hello.
...
В составе шлюза приложений имеется 3 стандартные политики, которыми можно воспользоваться. Hello `Get-AzureRmApplicationGatewaySslPredefinedPolicy` командлет извлекает эти политики. В каждой политике включены разные версии протокола SSL и комплекты шифров. Можно использовать эти предопределенные политики tooquickly настроить политику SSL на шлюзе приложения. Если определенная политика SSL не выбрана, по умолчанию выбирается политика **AppGwSslPolicy20170401**.

Hello ниже приведен пример выполнения `Get-AzureRmApplicationGatewaySslPredefinedPolicy`.
...
Следующий пример Hello задает пользовательскую политику SSL на шлюз приложений. Задает минимальный семейству hello слишком`TLSv1_1` и hello включает следующие комплекты шифров:

* TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
* TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256

> [!IMPORTANT]
> Комплект шифров по крайней мере один из hello после списка должен быть выбран при настройке пользовательской политики SSL. Для управления серверной частью шлюз приложений использует комплекты шифров RSA SHA256.
> * TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 
> * TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256
> * TLS_DHE_RSA_WITH_AES_128_GCM_SHA256
> * TLS_RSA_WITH_AES_128_GCM_SHA256
...
Hello следующий пример создает новый шлюз приложения с предварительно определенные политики SSL.
...
Посетите [Обзор перенаправления шлюза приложения](application-gateway-redirect-overview.md) toolearn как tooredirect HTTP-трафик tooa конечная точка HTTPS.
