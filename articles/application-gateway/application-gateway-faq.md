...
Правила обрабатываются в порядке создания. Рекомендуется настроить правила для нескольких сайтов прежде, чем базовые правила. Если сначала настроить прослушиватели для нескольких сайтов, это снизит вероятность направления трафика к несоответствующей серверной части. Эта проблема маршрутизации может возникнуть, так как базовое правило будет сопоставлять трафик по порту прежде, чем будет применено правило для нескольких сайтов.
...
* Разрешить пробы Azure Load Balancer (тег AzureLoadBalancer) и входящего виртуального сетевого трафика (тег VirtualNetwork) в [группах безопасности сети](../virtual-network/virtual-networks-nsg.md).
