...
Если это еще не сделано, добавление пользовательских устройств tooyour удаленного мониторинга решения. Выполните следующие шаги на панели мониторинга решения hello hello.
...
1. Установите переключатель **Позвольте мне определить собственный идентификатор устройства**. Введите идентификатор устройства, например **rasppi**, нажмите кнопку **проверить идентификатор** tooverify еще не использовалась hello имя решения и нажмите кнопку **создать** tooprovision hello устройства.

   ![Добавление идентификатора устройства][3]

1. Сделать устройство hello Примечание учетные данные (**идентификатор устройства**, **имя узла концентратора IoT**, и **ключ устройства**). Клиентское приложение на hello Raspberry Pi должен toohello tooconnect эти значения удаленного решением для мониторинга. Затем нажмите кнопку **Done**(Готово).
